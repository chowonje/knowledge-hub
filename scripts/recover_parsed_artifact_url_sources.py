#!/usr/bin/env python
"""Plan or apply a bounded URL PDF source recovery tranche."""
...
DEFAULT_OUTPUT_DIR = Path(
    "~/.khub/reports/parsed-artifact-coverage/2026-05-20/parsed-artifact-url-source-recovery"
).expanduser()
...
def main() -> int:
    ...
    parser.add_argument("--report-name", default="parsed-artifact-url-source-recovery")
    parser.add_argument("--limit", type=int, default=DEFAULT_URL_SOURCE_RECOVERY_LIMIT)
    parser.add_argument("--max-pdf-bytes", type=int, default=DEFAULT_MAX_SOURCE_PDF_BYTES)
    parser.add_argument("--apply", action="store_true", help="Download selected URL PDFs and register local source paths.")
    parser.add_argument("--probe-network", action="store_true", help="HEAD-probe resolved URL PDFs before reporting readiness.")
    parser.add_argument("--allow-network", action="store_true", help="Required with --apply or --probe-network because URL recovery is external.")
    ...
    if (args.apply or args.probe_network) and not args.allow_network:
        parser.error("--apply or --probe-network requires --allow-network")
    ...
    paths = write_parsed_artifact_url_source_recovery(report, args.output_dir)
    print(
        json.dumps(
            {
                ...
                "sourceUrlPresenceTaxonomy": report["sourceUrlPresenceTaxonomy"],
                "expectedCoverageChangeIfMaterialized": report["expectedCoverageChangeIfMaterialized"],
                "sourceUrlProbeAttemptedRows": sum(1 for item in report["items"] if item.get("sourceUrlProbeAttempted") is True),
