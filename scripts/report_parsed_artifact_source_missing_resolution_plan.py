#!/usr/bin/env python
"""Build a report-only parsed artifact source-missing resolution plan."""
...
DEFAULT_OUTPUT_DIR = Path(
    "~/.khub/reports/parsed-artifact-coverage/2026-05-20/parsed-artifact-source-missing-resolution-plan"
).expanduser()
...
def main() -> int:
    ...
    parser.add_argument("--plan-name", default="parsed-artifact-source-missing-resolution-plan")
    parser.add_argument("--limit", type=int, default=DEFAULT_SOURCE_MISSING_LIMIT)
    parser.add_argument("--max-pdf-bytes", type=int, default=None)
    ...
    paths = write_parsed_artifact_source_missing_resolution_plan(report, args.output_dir)
    print(
        json.dumps(
            {
                ...
                "nextRecommendedCoverageTranche": report["nextRecommendedCoverageTranche"],
