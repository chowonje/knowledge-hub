...
- AskV2 and EvidenceAssembly now accept document-level dates, version/release markers, and arXiv identifiers as temporal grounding, but still reject local `updated_at` / observed-only crawl timestamps on their own.
- Current full no-generation baseline: `python eval/knowledgeos/scripts/run_evidence_contract_perf_gate.py --live-stub-llm --run-profile full --json --timeout-sec 10` produced `status=failed`, `22/24` passed, `contractValidRate=1.0`, `citationGradeCoverageRate=1.0`, `abstainCorrectRate=1.0`, `verificationPassRate=null`, `verificationPassRateRaw=0.0`, `timeoutCount=0`, `latencyMs.p95=1062.613ms`, `generationDependencyRate=0.583333`, and `failureCategories={"provider/corpus_dependency": 2}`.
...
- `python eval/knowledgeos/scripts/run_evidence_contract_perf_gate.py --live-stub-llm --run-profile full --json --timeout-sec 10` (`status=failed`, `22/24` cases passed; contract/citation/abstain/timeout thresholds green; remaining failures are `vault_temporal_004` and `web_general_012`)
...
- `python -m pytest tests/test_answer_contracts_runtime.py tests/test_answer_contract_schemas.py tests/test_answer_quality_gate.py tests/test_retrieval_span_golden.py tests/test_evidence_contract_perf_gate.py tests/test_evidence_assembly_temporal.py tests/test_rag_answer_evidence.py -q` (`45 passed`)
- `python -m pytest tests/test_paper_ask_v2.py tests/test_ask_v2_sources.py -q` (`71 passed`)
...
- `python -m ruff check knowledge_hub/ai/evidence_assembly.py knowledge_hub/ai/ask_v2_verification.py tests/test_evidence_assembly_temporal.py` (`passed`)
