...
- Evidence / answer contract audit follow-up: runtime EvidencePacket and AnswerContract now apply the same strict source-provenance rule, so answer citations require real `sourceContentHash` / `source_content_hash` plus exact `chars:start-end` offsets. `contentHash` remains snippet metadata, loose locators stay non-strict, current-source resolution ignores snippet hashes, and answers abstain when no strict citations survive.
- Claim/section answer hardening follow-up: weak or unsupported claim cards no longer appear under the prompt's strict verified claim section, weak/unsupported claim consensus is no longer advisory-only for single-paper lookup, and section-native answer inputs require source excerpts before using materialized section cards as the native answer path.
- Final validation follow-up: the answer-quality golden fixture now expects signal-only grounding to be `coverage_status=insufficient` with `abstain=true`, matching the stricter no-strict-citation AnswerContract behavior.
