...
def _parsed_root(root: Path) -> Path:
    ...
    (parsed / "paper-1" / "document.md").write_text(
        "The compact canonical equation is FFN(x) = max(0, xW1 + b1)W2 + b2.\n"
        "First repeated text has Alpha Beta Gamma.\n"
        "Second repeated text has Alpha Beta Gamma.\n",
        encoding="utf-8",
    )
    return parsed


def test_tex_equation_canonical_text_normalizer_design_compares_profiles_and_validates_schema(
    tmp_path: Path,
) -> None:
    ...
    report_path = _diagnostic_report(
        tmp_path,
        [
            _diagnostic_row("diagnostic:0001", r"\mathrm{FFN}(x)=\max(0, xW_1 + b_1) W_2 + b_2"),
    ...
    assert payload["schema"] == TEX_EQUATION_CANONICAL_TEXT_NORMALIZER_DESIGN_SCHEMA_ID
    ...
    assert payload["counts"]["normalizerProfileCount"] == 2
    assert payload["counts"]["bestUniqueWindowRows"] == 1
    assert payload["counts"]["bestAmbiguousWindowRows"] == 1
    assert payload["counts"]["bestFailedWindowRows"] == 1
    assert payload["counts"]["strictEligibleRows"] == 0

    by_id = {row["source_diagnostic_id"]: row for row in payload["rows"]}
    assert by_id["diagnostic:0001"]["recommended_status"] == "unique_normalized_window_candidate_only"
    compact_profile = [
        result
        for result in by_id["diagnostic:0001"]["profile_results"]
        if result["profile_name"] == "canonical_math_compaction_v1"
    ][0]
    assert "xW1" in compact_profile["normalized_terms"]
    assert "b1" in compact_profile["normalized_terms"]
    assert by_id["diagnostic:0002"]["recommended_status"] == "ambiguous_normalized_window_candidate_only"
    assert by_id["diagnostic:0003"]["recommended_status"] == "normalized_window_not_found"
...
def test_tex_equation_canonical_text_normalizer_design_never_creates_evidence_or_source_spans(
    ...
    assert "normalized_windows_are_diagnostic_not_provenance" in row["strict_blockers"]


def test_tex_equation_canonical_text_normalizer_design_blocks_wrong_parent_schema(tmp_path: Path) -> None:
    parsed = _parsed_root(tmp_path)
    report_path = _diagnostic_report(
        tmp_path,
...
def test_tex_equation_canonical_text_normalizer_design_writer_outputs_schema_valid_reports(
    tmp_path: Path,
) -> None:
    parsed = _parsed_root(tmp_path)
    report_path = _diagnostic_report(tmp_path, [_diagnostic_row("diagnostic:0001", "Alpha Beta Gamma")])
    ...
    paths = write_tex_equation_canonical_text_normalizer_design_reports(payload, tmp_path / "reports")
    ...
    assert validate_payload(report, TEX_EQUATION_CANONICAL_TEXT_NORMALIZER_DESIGN_SCHEMA_ID, strict=True).ok
