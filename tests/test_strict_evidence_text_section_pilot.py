...
def test_text_section_pilot_marks_section_rows_and_holds_out_figure(tmp_path: Path) -> None:
    ...
    assert report["schema"] == STRICT_EVIDENCE_TEXT_SECTION_PILOT_SCHEMA_ID
    assert report["status"] == "ok"
    assert report["counts"]["inputRows"] == 2
    assert report["counts"]["sectionPilotInputRows"] == 1
    assert report["counts"]["textSectionPilotCandidateOnlyRows"] == 1
    assert report["counts"]["heldOutNonSectionRows"] == 1
    assert report["gate"]["textSectionPilotReady"] is True
    assert report["gate"]["figureRowsProcessedAsActivePilot"] is False
...
def test_text_section_pilot_blocks_hash_contract_violation(tmp_path: Path) -> None:
    store_path = tmp_path / "strict.jsonl"
...
def test_text_section_pilot_blocks_invalid_input_schema(tmp_path: Path) -> None:
    tranche_plan = _synthetic_tranche_plan_report(
        [_synthetic_tranche_plan_row(store_path=tmp_path / "strict.jsonl", index=1)]
    )
    ...
    assert report["gate"]["textSectionPilotReady"] is False
    assert validate_payload(
        report,
        STRICT_EVIDENCE_TEXT_SECTION_PILOT_SCHEMA_ID,
        strict=True,
...
def test_text_section_pilot_integrated_from_policy_gate_chain(tmp_path: Path) -> None:
    from knowledge_hub.papers.parsed_artifact_strict_evidence_policy_gate import (
    ...
    policy_gate_dir = tmp_path / "policy-gate"
    write_parsed_artifact_strict_evidence_policy_gate_reports(policy_gate_report, policy_gate_dir)
    tranche_plan_report = build_parsed_artifact_strict_evidence_promotion_tranche_plan(
        policy_gate_report_path=policy_gate_dir / "parsed-artifact-strict-evidence-policy-gate.json",
    )
    tranche_plan_dir = tmp_path / "tranche-plan"
    write_parsed_artifact_strict_evidence_promotion_tranche_plan_reports(
        tranche_plan_report,
        tranche_plan_dir,
    )
    ...
    report = build_strict_evidence_text_section_pilot(
        tranche_plan_report_path=tranche_plan_dir
        / "parsed-artifact-strict-evidence-promotion-tranche-plan.json",
    )

    assert report["status"] == "ok"
    assert report["counts"]["textSectionPilotCandidateOnlyRows"] == 2
    assert report["counts"]["heldOutNonSectionRows"] == 0
    assert report["counts"]["strictEvidenceWriteRows"] == 0


def test_text_section_pilot_writer_outputs_schema_valid_reports(tmp_path: Path) -> None:
    store_path = tmp_path / "strict.jsonl"
    ...
    paths = write_strict_evidence_text_section_pilot_reports(report, tmp_path / "reports")
