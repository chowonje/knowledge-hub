...
def _strict_evidence_record(*, index: int = 1, artifact_type: str = "figure") -> dict:
...
def _synthetic_dry_run_row(
    *,
    ...
    planned_tranche = (
        ...
        else TRANCHE_TEXT_SECTION_PILOT
    )
    ...
    return {
        ...
        "pilot_status": "figure_caption_pilot_candidate_only",
        ...
        "planned_executor_key": f"figure-caption-pilot-executor:strict-evidence:paper-1:{artifact_type}:{index}",
        ...
        "recommended_action": "queue_for_strict_evidence_figure_caption_pilot_executor_apply",
    }
...
def _synthetic_dry_run_report(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "input": {
            "figureCaptionPilotReportPath": "/tmp/figure-caption-pilot.json",
            "figureCaptionPilotSchema": "knowledge-hub.paper.strict-evidence-figure-caption-pilot.v1",
            "requestedPaperIds": [],
            "targetTranche": TRANCHE_FIGURE_CAPTION_PILOT,
        },
        "counts": {
            "inputRows": len(rows),
            "figureCaptionPilotCandidateRows": ready_rows,
            "dryRunReadyFigureCaptionPilotExecutorOnlyRows": ready_rows,
            "heldOutSectionRows": held_rows,
            "blockedInputReportSchemaViolationRows": 0,
            "blockedFigureCaptionPilotNotCandidateRows": 0,
        ...
        "gate": {
            "dryRunReadyForFigureCaptionPilotExecutor": ready_rows > 0,
            ...
            "schemaViolations": [],
            "decision": "strict_evidence_figure_caption_pilot_executor_dry_run_ready",
            "recommendedNextTranche": "strict_evidence_figure_caption_pilot_executor_apply",
        },
        "policy": {
            "reportOnly": True,
            "dryRunOnly": True,
...
def test_apply_plans_figure_ready_and_holds_out_section(tmp_path: Path) -> None:
    dry_run_path = tmp_path / "dry-run.json"
    _write_json(
        dry_run_path,
        _synthetic_dry_run_report(
            [
                _synthetic_dry_run_row(index=1, artifact_type="figure"),
    ...
    assert report["schema"] == STRICT_EVIDENCE_FIGURE_CAPTION_PILOT_EXECUTOR_APPLY_SCHEMA_ID
    ...
    assert report["gate"]["sectionRowsProcessedAsActiveApply"] is False
...
def test_apply_writes_manifest_only_when_explicit_apply(tmp_path: Path) -> None:
    ...
    _write_json(
        dry_run_path,
        _synthetic_dry_run_report([_synthetic_dry_run_row(index=1, artifact_type="figure")]),
    )
    ...
    report = build_strict_evidence_figure_caption_pilot_executor_apply(
        ...
        run_id="figure-caption-pilot-apply-test",
        apply=True,
    )
    ...
    assert manifest["schema"] == STRICT_EVIDENCE_FIGURE_CAPTION_PILOT_EXECUTOR_APPLY_SCHEMA_ID
...
def test_apply_integrated_from_executor_dry_run_builder(tmp_path: Path) -> None:
    ...
    from tests.test_strict_evidence_figure_caption_pilot_executor_dry_run import (
        _synthetic_figure_caption_pilot_report,
        _synthetic_figure_caption_pilot_row,
    ...
    _write_figure_caption_pilot_report(
        ...
        _synthetic_figure_caption_pilot_report(
            [
                ...
                _synthetic_figure_caption_pilot_row(store_path=store_path, index=2, artifact_type="figure"),
            ]
        ),
    )
    dry_run_report = build_strict_evidence_figure_caption_pilot_executor_dry_run(
...
def test_apply_writer_outputs_schema_valid_reports(tmp_path: Path) -> None:
    ...
    paths = write_strict_evidence_figure_caption_pilot_executor_apply_reports(
        report,
        tmp_path / "reports",
    )
