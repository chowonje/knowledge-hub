...
def _write_section_pilot_report(path: Path, report: dict) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def _synthetic_section_pilot_row(
    *,
    store_path: Path,
    index: int = 1,
    artifact_type: str = "section",
    ...
    pilot_status = (
        PILOT_STATUS_CANDIDATE_ONLY
        if artifact_type == "section"
        else PILOT_STATUS_HELD_OUT_NON_SECTION
    )
    return {
        ...
        "recommended_action": "queue_for_strict_evidence_text_section_pilot_executor_dry_run",
    }


def _synthetic_section_pilot_report(rows: list[dict]) -> dict:
    section_rows = sum(1 for row in rows if row.get("pilot_status") == PILOT_STATUS_CANDIDATE_ONLY)
    held_rows = sum(1 for row in rows if row.get("pilot_status") == PILOT_STATUS_HELD_OUT_NON_SECTION)
    return {
        ...
        "counts": {
            "inputRows": len(rows),
            "sectionPilotInputRows": section_rows,
            "textSectionPilotCandidateOnlyRows": section_rows,
        ...
        "gate": {
            "textSectionPilotReady": section_rows == len(rows) or section_rows > 0,
            ...
            "schemaViolations": [],
            "decision": "strict_evidence_text_section_pilot_ready",
            "recommendedNextTranche": TRANCHE_TEXT_SECTION_PILOT,
        },
        "policy": {
            "reportOnly": True,
...
def test_executor_dry_run_marks_section_ready_and_holds_out_figure(tmp_path: Path) -> None:
    ...
    _write_section_pilot_report(
        ...
        _synthetic_section_pilot_report(
            [
                ...
                _synthetic_section_pilot_row(store_path=store_path, index=2, artifact_type="figure"),
            ]
        ),
    )
    ...
    assert report["schema"] == STRICT_EVIDENCE_TEXT_SECTION_PILOT_EXECUTOR_DRY_RUN_SCHEMA_ID
    assert report["status"] == "ok"
    assert report["counts"]["dryRunReadyTextSectionPilotExecutorOnlyRows"] == 1
    assert report["counts"]["heldOutNonSectionRows"] == 1
    assert report["gate"]["dryRunReadyForTextSectionPilotExecutor"] is True
    assert report["gate"]["figureRowsProcessedAsActiveExecutor"] is False
...
def test_executor_dry_run_blocks_hash_contract_violation(tmp_path: Path) -> None:
    store_path = tmp_path / "strict.jsonl"
    record = _strict_evidence_record(index=1, artifact_type="section")
    record["verbatimSubstringSha256"] = "mismatch"
    _write_jsonl(store_path, [record])
    pilot_row = _synthetic_section_pilot_row(store_path=store_path, index=1, artifact_type="section")
    pilot_row["verbatimSubstringSha256"] = "mismatch"
    pilot_path = tmp_path / "section-pilot.json"
    _write_section_pilot_report(pilot_path, _synthetic_section_pilot_report([pilot_row]))
    ...
    assert report["rows"][0]["dry_run_status"] == DRY_RUN_STATUS_BLOCKED_HASH_CONTRACT
    assert validate_payload(
        report,
        STRICT_EVIDENCE_TEXT_SECTION_PILOT_EXECUTOR_DRY_RUN_SCHEMA_ID,
        strict=True,
...
def test_executor_dry_run_blocks_invalid_input_schema(tmp_path: Path) -> None:
    pilot_report = _synthetic_section_pilot_report(
        [_synthetic_section_pilot_row(store_path=tmp_path / "strict.jsonl", index=1)]
    )
    pilot_report["schema"] = "wrong.schema"
    pilot_path = tmp_path / "section-pilot.json"
    _write_section_pilot_report(pilot_path, pilot_report)
    ...
    assert report["gate"]["dryRunReadyForTextSectionPilotExecutor"] is False
    assert validate_payload(
        report,
        STRICT_EVIDENCE_TEXT_SECTION_PILOT_EXECUTOR_DRY_RUN_SCHEMA_ID,
        strict=True,
...
def test_executor_dry_run_integrated_from_section_pilot_builder(tmp_path: Path) -> None:
    from knowledge_hub.papers.strict_evidence_text_section_pilot import (
        build_strict_evidence_text_section_pilot,
        write_strict_evidence_text_section_pilot_reports,
    )
    from tests.test_strict_evidence_text_section_pilot import (
    ...
    _write_tranche_plan_report(
        ...
        _synthetic_tranche_plan_report(
            [
                _synthetic_tranche_plan_row(store_path=store_path, index=1, artifact_type="section"),
                _synthetic_tranche_plan_row(store_path=store_path, index=2, artifact_type="section"),
            ]
        ),
    )
    section_pilot_report = build_strict_evidence_text_section_pilot(
        tranche_plan_report_path=tranche_plan_path,
    )
    section_pilot_dir = tmp_path / "section-pilot"
    write_strict_evidence_text_section_pilot_reports(section_pilot_report, section_pilot_dir)

    report = build_strict_evidence_text_section_pilot_executor_dry_run(
        section_pilot_report_path=section_pilot_dir / "strict-evidence-text-section-pilot.json",
    )

    assert report["status"] == "ok"
    assert report["counts"]["dryRunReadyTextSectionPilotExecutorOnlyRows"] == 2
...
def test_executor_dry_run_writer_outputs_schema_valid_reports(tmp_path: Path) -> None:
    store_path = tmp_path / "strict.jsonl"
    _write_jsonl(store_path, [_strict_evidence_record(index=1, artifact_type="section")])
    pilot_path = tmp_path / "section-pilot.json"
    _write_section_pilot_report(
        pilot_path,
        _synthetic_section_pilot_report(
