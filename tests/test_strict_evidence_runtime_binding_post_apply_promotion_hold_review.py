...
from tests.test_strict_evidence_runtime_binding_executor_apply_readback_review import (
    _fixture_reports,
)
...
def _readback_report_path(tmp_path: Path) -> Path:
    ...
    readback = build_strict_evidence_runtime_binding_executor_apply_readback_review(
    ...
    return readback_dir / "strict-evidence-runtime-binding-executor-apply-readback-review.json"
...
def test_post_apply_hold_review_marks_readback_validated_rows_on_hold(tmp_path: Path) -> None:
    ...
    assert counts["runtimeBindingRecordRows"] == 1
    assert counts["readbackValidatedRows"] == 1
    assert counts["runtimeBindingHoldRows"] == 1
    assert counts["sectionHoldRows"] == 1
    assert counts["figureCaptionHoldRows"] == 0
    assert counts["runtimeBindingRecordWriteAllowedRows"] == 0
    assert counts["runtimeVisibleAllowedRows"] == 0
...
def test_post_apply_hold_review_blocks_invalid_readback_report(tmp_path: Path) -> None:
    ...
    assert report["gate"]["holdDecision"] == (
        "strict_evidence_runtime_binding_post_apply_promotion_hold_blocked"
    )
...
def test_post_apply_hold_review_blocks_illegal_row_flag_without_masking_it(tmp_path: Path) -> None:
    ...
    assert report["status"] == "blocked"
    assert report["rows"][0]["hold_status"] == HOLD_STATUS_BLOCKED_RUNTIME_OR_ANSWER
...
def test_post_apply_hold_review_writer_outputs_schema_valid_reports(tmp_path: Path) -> None:
    ...
    assert written["schema"] == STRICT_EVIDENCE_RUNTIME_BINDING_POST_APPLY_PROMOTION_HOLD_REVIEW_SCHEMA_ID
    assert summary["counts"]["runtimeBindingHoldRows"] == 1
    assert "Strict Evidence Runtime Binding Post-Apply Promotion Hold Review" in markdown
    assert validate_payload(
        written,
        STRICT_EVIDENCE_RUNTIME_BINDING_POST_APPLY_PROMOTION_HOLD_REVIEW_SCHEMA_ID,
        strict=True,
