...
from tests.test_strict_evidence_citation_grade_executor_apply_readback_review import (
    _fixture_reports,
)
...
def _readback_report_path(tmp_path: Path) -> Path:
    ...
    readback = build_strict_evidence_citation_grade_executor_apply_readback_review(
    ...
    return readback_dir / "strict-evidence-citation-grade-executor-apply-readback-review.json"
...
def test_post_apply_hold_review_marks_readback_validated_rows_on_hold(tmp_path: Path) -> None:
    ...
    assert counts["citationGradeRecordRows"] == 1
    assert counts["readbackValidatedRows"] == 1
    assert counts["citationGradeHoldRows"] == 1
    assert counts["sectionHoldRows"] == 1
    assert counts["figureCaptionHoldRows"] == 0
    assert counts["citationGradeRecordWriteAllowedRows"] == 0
    assert counts["citationGradeBooleanMutationAllowedRows"] == 0
    ...
    assert counts["citationGradeBooleanMutationRows"] == 0
    assert report["rows"][0]["hold_status"] == HOLD_STATUS_ACTIVE
    assert report["gate"]["holdDecision"] == (
...
def test_post_apply_hold_review_blocks_invalid_readback_report(tmp_path: Path) -> None:
    ...
    assert report["gate"]["holdDecision"] == (
        "strict_evidence_citation_grade_post_apply_promotion_hold_blocked"
    )
...
def test_post_apply_hold_review_blocks_illegal_row_flag_without_masking_it(tmp_path: Path) -> None:
    ...
    assert "writeMatrix.runtimeEvidenceCreated_true" in report["rows"][0]["hold_blockers"]
...
def test_post_apply_hold_review_blocks_downstream_gate_enablement(tmp_path: Path) -> None:
    readback_path = _readback_report_path(tmp_path)
    payload = json.loads(readback_path.read_text(encoding="utf-8"))
    payload["gate"]["runtimeEvidenceReady"] = True
    _write_json(readback_path, payload)
    ...
    assert "downstream_gate_runtimeEvidence_already_enabled" in report["rows"][0]["hold_blockers"]
...
def test_post_apply_hold_review_blocks_store_count_drift(tmp_path: Path) -> None:
    readback_path = _readback_report_path(tmp_path)
    payload = json.loads(readback_path.read_text(encoding="utf-8"))
    payload["counts"]["eligibilityStoreRows"] = 2
    _write_json(readback_path, payload)
    ...
    assert "eligibilityStoreRows=2_expected_1" in report["rows"][0]["hold_blockers"]


def test_post_apply_hold_review_blocks_input_schema_violation_rows(tmp_path: Path) -> None:
    readback_path = _readback_report_path(tmp_path)
    payload = json.loads(readback_path.read_text(encoding="utf-8"))
    ...
    assert report["status"] == "blocked"
    assert report["rows"][0]["hold_status"] == HOLD_STATUS_BLOCKED_INPUT_SCHEMA
...
def test_post_apply_hold_review_writer_outputs_schema_valid_reports(tmp_path: Path) -> None:
    ...
    assert written["schema"] == STRICT_EVIDENCE_CITATION_GRADE_POST_APPLY_PROMOTION_HOLD_REVIEW_SCHEMA_ID
    assert summary["counts"]["citationGradeHoldRows"] == 1
    assert "Strict Evidence Citation-Grade Post-Apply Promotion Hold Review" in markdown
    assert validate_payload(
        written,
        STRICT_EVIDENCE_CITATION_GRADE_POST_APPLY_PROMOTION_HOLD_REVIEW_SCHEMA_ID,
        strict=True,
...
def test_post_apply_hold_review_integrated_measured_local_report() -> None:
    ...
    assert report["counts"]["citationGradeHoldRows"] == 99
