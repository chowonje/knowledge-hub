...
def test_text_only_scope_gate_blocks_missing_phase_reports_with_explicit_blockers(tmp_path: Path) -> None:
    _write_json(
        tmp_path / "source_alias_normalization.v1.json",
        {
            "schema": "knowledge-hub.paper.source-alias-normalization-report.v1",
            "status": "ready",
            "caseRows": 10,
            "unsafeDirectAliasRows": 0,
            "expectationFailureRows": 0,
            "privatePathLeakRows": 0,
            "strictEvidencePromotionRows": 0,
            "runtimeAnswerVisibleExposureRows": 0,
            "rows": [{"caseId": "source-alias"}],
        },
    )
    ...
    assert report["status"] == "blocked"
    assert report["textReadyRows"] == 1
    assert report["textHoldRows"] == 5
    assert report["blockerRows"] == 6
    assert report["nextAction"] == "replay_missing_text_evidence_phase_reports"
    blocker_ids = {row["blockerId"] for row in report["blockers"]}
    assert "missing_phase_report:text_section_paragraph_span_artifacts" in blocker_ids
    assert "missing_convergence_report" in blocker_ids
    assert validate_payload(report, TEXT_EVIDENCE_RC_TEXT_ONLY_SCOPE_GATE_SCHEMA_ID, strict=True).ok
