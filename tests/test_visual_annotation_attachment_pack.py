...
def _make_pdf(path: Path) -> None:
    ...
    page.insert_text((72, 80), "Figure 1: synthetic visual region")
    page.draw_rect(fitz.Rect(70, 100, 220, 190), color=(0, 0, 0), fill=(0.8, 0.9, 1.0))
    document.save(str(path))
    document.close()


def _web_pack(pdf_hash: str, *, paper_ref: str = "papers_dir/sample.pdf") -> dict[str, object]:
    return {
        ...
        "packRowsDetail": [
            {
                ...
                "packCandidateId": "visual-annotation-web-pack:test:1111111111111111",
                "sourceCandidateId": "visual-layout:sample-paper:figure_caption_region:1:1111111111111111",
                "paperId": "sample-paper",
                "paperRef": paper_ref,
                "sourceContentHash": pdf_hash,
                "page": 1,
                "bbox": [70.0, 70.0, 220.0, 110.0],
...
def test_attachment_pack_renders_local_pdf_crop_and_validates_schema(tmp_path: Path) -> None:
    ...
    report = build_visual_annotation_attachment_pack(
        _web_pack(sha256_file(pdf_path)),
        papers_root=papers_root,
        output_dir=tmp_path / "out",
        output_dir_ref="eval/knowledgeos/reports/visual_annotation_attachment_pack_001",
        generated_at="2026-05-26T00:00:00Z",
    )

    assert report["status"] == "ready"
    assert report["decision"] == "ready_for_manual_web_vlm_run"
    ...
    assert row["attachmentRef"].startswith("eval/knowledgeos/reports/visual_annotation_attachment_pack_001/assets/")
...
def test_attachment_pack_blocks_missing_pdf_without_external_download(tmp_path: Path) -> None:
    report = build_visual_annotation_attachment_pack(
        _web_pack("sha256:" + "1" * 64),
...
def test_attachment_pack_writer_uses_sanitized_refs(tmp_path: Path) -> None:
    papers_root = tmp_path / "papers"
    papers_root.mkdir()
    pdf_path = papers_root / "sample.pdf"
    _make_pdf(pdf_path)
    ...
    write_visual_annotation_attachment_pack(report, report_json=report_json, report_md=report_md)
