...
from knowledge_hub.papers.sectionspan_pdf_offset_review_decision_record import (
...
def _template_row(row_id: str = "sectionspan-pdf-offset-review-decision:0001", **overrides: object) -> dict:
    payload = {
...
def _template(rows: list[dict], **overrides: object) -> dict:
    payload = {
        "schema": "knowledge-hub.paper.sectionspan-pdf-offset-review-decision-template.v1",
...
def test_decision_record_defaults_to_needs_review_without_decision_file(tmp_path: Path) -> None:
    template = _write(tmp_path, "template.json", _template([_template_row(), _template_row("row-2")]))

    payload = build_sectionspan_pdf_offset_review_decision_record(
        sectionspan_pdf_offset_review_decision_template_report=template
    )

    assert payload["schema"] == SECTIONSPAN_PDF_OFFSET_REVIEW_DECISION_RECORD_SCHEMA_ID
    assert validate_payload(payload, SECTIONSPAN_PDF_OFFSET_REVIEW_DECISION_RECORD_SCHEMA_ID, strict=True).ok
...
def test_decision_record_records_valid_decisions_without_creating_strict_evidence(tmp_path: Path) -> None:
    approve_id = "row-approve"
    reject_id = "row-reject"
    template = _write(tmp_path, "template.json", _template([_template_row(approve_id), _template_row(reject_id)]))
    decisions = _write(
        tmp_path,
        "decisions.json",
        {
            "decisions": [
                ...
                {"source_decision_row_id": reject_id, "decision": "reject_keep_candidate_only", "notes": "ambiguous"},
            ]
        },
    )
...
def test_decision_record_blocks_invalid_or_unknown_decisions(tmp_path: Path) -> None:
    ...
    assert "review_decision_unknown_template_row_id" in payload["gate"]["unsafeUpstreamFlags"]
    assert "review_decision_duplicate_row_id" in payload["gate"]["unsafeUpstreamFlags"]
    assert "review_decision_invalid_value" in payload["gate"]["unsafeUpstreamFlags"]


def test_decision_record_blocks_unsafe_template(tmp_path: Path) -> None:
    template = _write(
        tmp_path,
        "template.json",
        _template(
    ...
    assert payload["status"] == "blocked"
    assert "sectionspan_pdf_offset_review_decision_template_schema_mismatch" in payload["gate"]["unsafeUpstreamFlags"]
    assert "decisionTemplate_strictEvidenceCreated_true" in payload["gate"]["unsafeUpstreamFlags"]


def test_decision_record_writer_outputs_schema_valid_json_and_markdown(tmp_path: Path) -> None:
    template = _write(tmp_path / "input", "template.json", _template([_template_row()]))
    payload = build_sectionspan_pdf_offset_review_decision_record(
        sectionspan_pdf_offset_review_decision_template_report=template
    )

    paths = write_sectionspan_pdf_offset_review_decision_record_reports(payload, tmp_path / "reports")
    ...
    assert validate_payload(record, SECTIONSPAN_PDF_OFFSET_REVIEW_DECISION_RECORD_SCHEMA_ID, strict=True).ok
