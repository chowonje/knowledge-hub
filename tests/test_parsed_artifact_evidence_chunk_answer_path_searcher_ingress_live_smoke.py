...
from knowledge_hub.domain.ai_papers.query_plan import normalize_query_plan_dict
...
def _record(index: int, *, paper_id: str, artifact_type: str = "paragraph") -> dict[str, Any]:
    excerpt = f"{paper_id} searcher ingress evidence chunk {index} with source hash and offset support."
    start = index * 100
...
def test_query_plan_normalization_preserves_evidence_chunk_adapter_opt_in() -> None:
    normalized = normalize_query_plan_dict(
        {
            "family": "paper_lookup",
...
def test_searcher_ingress_live_smoke_ready_with_fake_local_llm(tmp_path: Path) -> None:
    ...
    _write_records(tmp_path, "paper-b", [_record(3, paper_id="paper-b", artifact_type="section"), _record(4, paper_id="paper-b")])

    report = build_parsed_artifact_evidence_chunk_answer_path_searcher_ingress_live_smoke(
        papers_dir=tmp_path,
    ...
    assert report["searcherPayload"]["status"] == "ok"
    assert report["searcherPayload"]["queryPlanOptInSnake"] == "runtime_v1"
    assert report["searcherPayload"]["queryPlanOptInCamel"] == "runtime_v1"
    assert report["adapterDiagnostics"]["status"] == "applied"
    ...
    assert report["gate"]["queryPlanOptInPreserved"] is True
    assert report["gate"]["runtimeAdapterApplied"] is True
    assert all(row["excerptIncludedInReport"] is False for row in report["rows"])
    assert "searcher ingress evidence chunk" not in json.dumps(report, ensure_ascii=False)
...
def test_searcher_ingress_live_smoke_blocks_when_candidate_store_missing(tmp_path: Path) -> None:
    report = build_parsed_artifact_evidence_chunk_answer_path_searcher_ingress_live_smoke(
        papers_dir=tmp_path,
...
def test_searcher_ingress_live_smoke_writer_outputs_schema_valid_reports(tmp_path: Path) -> None:
    _write_records(tmp_path, "paper-a", [_record(1, paper_id="paper-a"), _record(2, paper_id="paper-a")])
    report = build_parsed_artifact_evidence_chunk_answer_path_searcher_ingress_live_smoke(
        papers_dir=tmp_path,
        resolved_paper_ids=["paper-a"],
    ...
    paths = write_parsed_artifact_evidence_chunk_answer_path_searcher_ingress_live_smoke(
        report,
