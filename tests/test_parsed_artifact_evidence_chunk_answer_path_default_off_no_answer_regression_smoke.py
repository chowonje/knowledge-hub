...
def _record(index: int, *, paper_id: str, artifact_type: str = "paragraph") -> dict[str, Any]:
    excerpt = f"{paper_id} default-off evidence chunk {index} with source hash and offset support."
    start = index * 100
...
def _upstream_ready() -> dict[str, Any]:
    return {
        "status": "ready",
        "decision": SEARCHER_INGRESS_READY_DECISION,
        "counts": {"schemaViolationCount": 0},
    }


def test_default_off_no_answer_regression_smoke_ready(tmp_path: Path) -> None:
    ...
    report = build_parsed_artifact_evidence_chunk_answer_path_default_off_no_answer_regression_smoke(
        ...
        searcher_ingress_report=_upstream_ready(),
        generated_at="2026-05-29T00:00:00Z",
    )

    assert report["status"] == "ready"
    assert report["decision"] == READY_DECISION
    assert report["counts"]["inputScenarioRows"] == 3
    assert report["counts"]["passRows"] == 3
    assert report["counts"]["noAnswerRows"] == 3
    assert report["counts"]["answerableRows"] == 0
    assert report["counts"]["adapterAppliedRows"] == 0
    assert report["counts"]["adapterRowsAdded"] == 0
    assert report["counts"]["localFakeLlmCallRows"] == 0
    assert report["counts"]["externalLlmCallRows"] == 0
    assert report["gate"]["allScenariosNoAnswer"] is True
    assert report["gate"]["answerabilityStayedFalse"] is True
    assert report["gate"]["adapterNeverApplied"] is True
    assert {row["scenarioId"] for row in report["rows"]} == {
        "opt_in_absent",
        "opt_in_missing_resolved_paper",
        "opt_in_non_paper_source",
    }
    assert "default-off evidence chunk" not in json.dumps(report, ensure_ascii=False)
...
def test_default_off_no_answer_regression_blocks_when_upstream_not_ready(tmp_path: Path) -> None:
    report = build_parsed_artifact_evidence_chunk_answer_path_default_off_no_answer_regression_smoke(
        papers_dir=tmp_path,
        resolved_paper_ids=["paper-a"],
        searcher_ingress_report={"status": "blocked", "decision": "blocked", "counts": {"schemaViolationCount": 1}},
        generated_at="2026-05-29T00:00:00Z",
    )

    assert report["status"] == "blocked"
    assert "searcher_ingress_smoke_not_ready" in report["gate"]["schemaViolations"]
    assert validate_payload(
        report,
        PARSED_ARTIFACT_EVIDENCE_CHUNK_ANSWER_PATH_DEFAULT_OFF_NO_ANSWER_REGRESSION_SMOKE_SCHEMA_ID,
        strict=True,
...
def test_default_off_no_answer_regression_writer_outputs_schema_valid_reports(tmp_path: Path) -> None:
