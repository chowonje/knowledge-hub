...
- live compare reports `declaredCaseCount`, `evaluableCaseCount`, `coveragePct`, `skippedForMissingCorpus`, and `skippedForHashMismatch` next to the existing safety metrics. A headline such as `15/15` is complete only when the report also shows full corpus coverage.
