#!/usr/bin/env python3
"""Build labs vector-index apply review for visual retrieval hints."""
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/"
    "limited_visual_retrieval_hint_candidate_store_labs_vector_index_apply_review_005.v1.md"
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_limited_visual_retrieval_hint_candidate_store_labs_vector_index_apply_review(
        labs_vector_index_apply_executor_dry_run=load_json(source_path),
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "limited visual retrieval hint candidate-store labs vector-index apply review schema "
            "validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write_report:
        paths = write_limited_visual_retrieval_hint_candidate_store_labs_vector_index_apply_review(
            report,
