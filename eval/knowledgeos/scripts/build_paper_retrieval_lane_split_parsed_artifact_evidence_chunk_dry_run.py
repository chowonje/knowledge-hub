#!/usr/bin/env python3
"""Build dry-run report for the paper retrieval lane split."""
...
DEFAULT_SOURCE_FINAL_REVIEW_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/"
    "limited_visual_retrieval_hint_final_merge_review_005.v1.json"
)
DEFAULT_JSON_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/"
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/"
    "paper_retrieval_lane_split_parsed_artifact_evidence_chunk_dry_run.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--source-final-review", type=Path, default=DEFAULT_SOURCE_FINAL_REVIEW_PATH)
...
def main(argv: list[str] | None = None) -> int:
    ...
    source_final_review_path = args.source_final_review.expanduser()
    report = build_paper_retrieval_lane_split_parsed_artifact_evidence_chunk_dry_run(
        visual_retrieval_hint_final_merge_review=load_json(source_final_review_path),
        source_visual_retrieval_hint_final_merge_review_ref=sanitized_report_ref(
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "paper retrieval lane split parsed-artifact evidence chunk dry-run schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write:
        paths = write_paper_retrieval_lane_split_parsed_artifact_evidence_chunk_dry_run(
            report,
