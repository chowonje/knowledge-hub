...
DEFAULT_DESIGN_REPORT = (
    ...
    / "parsed-artifact-pymupdf-quality-repair-design.json"
)
DEFAULT_OUTPUT_DIR = (
    REPO_ROOT
    / "eval"
...
def build_report_from_inputs(args: argparse.Namespace) -> dict:
    try:
        ...
        return build_blocked_pymupdf_quality_repair_executor_dry_run(reason="design_report_unavailable")

    return build_pymupdf_quality_repair_executor_dry_run(design_report=design_report)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build a report-only PyMuPDF parsed-artifact quality repair executor dry-run."
    )
    ...
    validation = validate_payload(report, PYMUPDF_QUALITY_REPAIR_EXECUTOR_DRY_RUN_SCHEMA_ID, strict=True)
    if not validation.ok:
        report = build_blocked_pymupdf_quality_repair_executor_dry_run(reason="schema_validation_failed")

    if not args.no_write:
        paths = write_pymupdf_quality_repair_executor_dry_run_reports(report, args.output_dir)
        if not args.json:
            print(
                "wrote PyMuPDF quality repair executor dry-run: "
