#!/usr/bin/env python3
"""Build parsed-artifact evidence chunk candidate answerability policy gate."""
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/parsed_artifact_evidence_chunk_candidate_answerability_policy_gate.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--readback-review", type=Path, default=DEFAULT_READBACK_REVIEW_PATH)
...
def main(argv: list[str] | None = None) -> int:
    ...
    readback_path = args.readback_review.expanduser()
    report = build_parsed_artifact_evidence_chunk_candidate_answerability_policy_gate(
        full_apply_readback_review_report=load_json(readback_path),
        source_readback_review_report_ref=sanitized_report_ref(readback_path, project_root=PROJECT_ROOT),
    )
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "parsed artifact evidence chunk candidate answerability policy gate schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write_report:
        paths = write_parsed_artifact_evidence_chunk_candidate_answerability_policy_gate(
            report,
