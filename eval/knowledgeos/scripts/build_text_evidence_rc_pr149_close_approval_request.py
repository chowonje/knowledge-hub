#!/usr/bin/env python
"""Build the v0.1 text-evidence RC PR #149 close approval request."""
...
DEFAULT_JSON_PATH = REPORTS_ROOT / "text_evidence_rc_pr149_close_approval_request.v1.json"
DEFAULT_MD_PATH = REPORTS_ROOT / "text_evidence_rc_pr149_close_approval_request.v1.md"
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_text_evidence_rc_pr149_close_approval_request(reports_root=args.reports_root)
    if not args.no_write:
        write_report(report, json_path=args.out, markdown_path=args.md_out)
    if args.json:
        print(json.dumps(report, ensure_ascii=False, indent=2))
    return 0 if report.get("status") == "ready_for_user_decision" else 1
