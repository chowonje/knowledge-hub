#!/usr/bin/env python3
"""Build parsed-artifact evidence chunk labs opt-in surface live smoke."""
...
from knowledge_hub.papers.parsed_artifact_evidence_chunk_answer_path_labs_opt_in_surface_live_smoke import (
    DEFAULT_LABS_SURFACE_DESIGN_REPORT,
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/parsed_artifact_evidence_chunk_answer_path_labs_opt_in_surface_live_smoke.v1.md"
)
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--labs-surface-design-report", type=Path, default=DEFAULT_LABS_SURFACE_DESIGN_REPORT)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_parsed_artifact_evidence_chunk_answer_path_labs_opt_in_surface_live_smoke(
        ...
        query=args.query,
        labs_surface_design_report_path=args.labs_surface_design_report,
    )
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "parsed artifact evidence chunk labs opt-in surface live smoke schema validation failed: "
