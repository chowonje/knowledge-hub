#!/usr/bin/env python3
"""Build parsed-artifact evidence chunk labs opt-in user-test promotion review."""
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/parsed_artifact_evidence_chunk_answer_path_labs_opt_in_user_test_promotion_review.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--output-capture-report", type=Path, default=DEFAULT_OUTPUT_CAPTURE_REPORT)
    parser.add_argument("--product-definition", type=Path, default=DEFAULT_PRODUCT_DEFINITION)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_parsed_artifact_evidence_chunk_answer_path_labs_opt_in_user_test_promotion_review(
        output_capture_report_path=args.output_capture_report,
        product_definition_path=args.product_definition,
    )
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "parsed artifact evidence chunk labs opt-in user-test promotion review schema validation failed: "
