#!/usr/bin/env python3
"""Build parsed-artifact evidence chunk candidate runtime adapter design."""
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/parsed_artifact_evidence_chunk_candidate_runtime_adapter_design.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--contract-review", type=Path, default=DEFAULT_CONTRACT_REVIEW_PATH)
...
def main(argv: list[str] | None = None) -> int:
    ...
    contract_review_path = args.contract_review.expanduser()
    report = build_parsed_artifact_evidence_chunk_candidate_runtime_adapter_design(
        runtime_contract_review_report=load_json(contract_review_path),
        source_runtime_contract_review_report_ref=sanitized_report_ref(
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "parsed artifact evidence chunk candidate runtime adapter design schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write_report:
        paths = write_parsed_artifact_evidence_chunk_candidate_runtime_adapter_design(
            report,
