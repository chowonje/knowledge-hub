#!/usr/bin/env python3
"""Build parsed-artifact evidence chunk answer-path opt-in implementation design."""
...
DEFAULT_JSON_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/parsed_artifact_evidence_chunk_answer_path_opt_in_implementation_design.v1.json"
)
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/parsed_artifact_evidence_chunk_answer_path_opt_in_implementation_design.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--route-review-report", type=Path, default=PROJECT_ROOT / DEFAULT_ROUTE_REVIEW_REPORT)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_parsed_artifact_evidence_chunk_answer_path_opt_in_implementation_design(
        route_review_report_path=args.route_review_report,
    )
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "parsed artifact evidence chunk answer path opt-in implementation design schema validation failed: "
