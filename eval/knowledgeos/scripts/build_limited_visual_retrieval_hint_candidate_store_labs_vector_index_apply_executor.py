#!/usr/bin/env python3
"""Build or explicitly apply labs visual retrieval-hint vector-index records."""
...
DEFAULT_SOURCE_REVIEW_PATH = (
    ...
    "limited_visual_retrieval_hint_candidate_store_labs_vector_index_apply_review_005.v1.json"
)
...
DEFAULT_JSON_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/"
    "limited_visual_retrieval_hint_candidate_store_labs_vector_index_apply_executor_005.v1.json"
)
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/"
    "limited_visual_retrieval_hint_candidate_store_labs_vector_index_apply_executor_005.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--source-apply-review", type=Path, default=DEFAULT_SOURCE_REVIEW_PATH)
    parser.add_argument("--source-apply-executor-dry-run", type=Path, default=DEFAULT_SOURCE_DRY_RUN_PATH)
    ...
    parser.add_argument("--apply", action="store_true", help="Write labs vector-index JSONL records.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    source_review_path = args.source_apply_review.expanduser()
    source_dry_run_path = args.source_apply_executor_dry_run.expanduser()
    report = execute_limited_visual_retrieval_hint_candidate_store_labs_vector_index_apply_executor(
        labs_vector_index_apply_review=load_json(source_review_path),
        labs_vector_index_apply_executor_dry_run=load_json(source_dry_run_path),
        source_labs_vector_index_apply_review_ref=sanitized_report_ref(
            source_review_path,
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "limited visual retrieval hint candidate-store labs vector-index apply executor schema "
            "validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write_report:
        paths = write_limited_visual_retrieval_hint_candidate_store_labs_vector_index_apply_executor(
            report,
