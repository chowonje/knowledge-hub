#!/usr/bin/env python3
"""Build parsed-artifact evidence chunk candidate runtime integration dry-run."""
...
DEFAULT_ANSWERABILITY_POLICY_GATE_PATH = (
    PROJECT_ROOT
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/parsed_artifact_evidence_chunk_candidate_runtime_integration_dry_run.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--answerability-policy-gate", type=Path, default=DEFAULT_ANSWERABILITY_POLICY_GATE_PATH)
...
def main(argv: list[str] | None = None) -> int:
    ...
    policy_gate_path = args.answerability_policy_gate.expanduser()
    report = build_parsed_artifact_evidence_chunk_candidate_runtime_integration_dry_run(
        answerability_policy_gate_report=load_json(policy_gate_path),
        source_answerability_policy_gate_report_ref=sanitized_report_ref(policy_gate_path, project_root=PROJECT_ROOT),
    )
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "parsed artifact evidence chunk candidate runtime integration dry-run schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write_report:
        paths = write_parsed_artifact_evidence_chunk_candidate_runtime_integration_dry_run(
            report,
