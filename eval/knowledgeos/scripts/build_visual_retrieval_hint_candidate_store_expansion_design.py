#!/usr/bin/env python3
"""Build the report-only visual retrieval hint candidate-store expansion design."""
...
DEFAULT_SOURCE_VALIDATION_PATH = (
    PROJECT_ROOT / "eval/knowledgeos/reports/visual_annotation_expansion_web_output_002.validation.v1.json"
)
DEFAULT_JSON_PATH = (
    PROJECT_ROOT / "eval/knowledgeos/reports/visual_retrieval_hint_candidate_store_expansion_design.v1.json"
)
DEFAULT_MD_PATH = (
    PROJECT_ROOT / "eval/knowledgeos/reports/visual_retrieval_hint_candidate_store_expansion_design.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--source-validation-report", type=Path, default=DEFAULT_SOURCE_VALIDATION_PATH)
...
def main(argv: list[str] | None = None) -> int:
    ...
    source_path = args.source_validation_report.expanduser()
    validation_report = load_json(source_path)
    report = build_visual_retrieval_hint_candidate_store_expansion_design(
        validation_report,
        source_validation_report_ref=sanitized_report_ref(source_path, project_root=PROJECT_ROOT),
    )
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "visual retrieval hint candidate-store design schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write:
        paths = write_visual_retrieval_hint_candidate_store_expansion_design(
            report,
