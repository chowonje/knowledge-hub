#!/usr/bin/env python3
"""Build a report-only visual retrieval hint usefulness proxy eval."""
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT / "eval/knowledgeos/reports/visual_retrieval_hint_usefulness_eval.v1.md"
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_visual_retrieval_hint_usefulness_eval(layout_report, dry_run_reports)
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "visual retrieval hint usefulness eval schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write:
        paths = write_visual_retrieval_hint_usefulness_eval(
            report,
