#!/usr/bin/env python
"""Build the FigureCaptionArtifact vertical-slice report."""
...
DEFAULT_JSON_PATH = PROJECT_ROOT / "eval" / "knowledgeos" / "reports" / "figure_caption_artifact_vertical_slice.v1.json"
DEFAULT_MD_PATH = PROJECT_ROOT / "eval" / "knowledgeos" / "reports" / "figure_caption_artifact_vertical_slice.v1.md"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--no-write", action="store_true", help="Build the report without writing JSON/MD files.")
    return parser.parse_args(argv)


def main(argv: list[str] | None = None) -> int:
    args = _parse_args(list(argv or sys.argv[1:]))
    report = build_vertical_slice_report(
