#!/usr/bin/env python3
"""Build the visual retrieval hint expansion human/product decision record."""
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/visual_retrieval_hint_candidate_store_expansion_human_product_decision_record.v1.md"
)
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--decision-file", type=Path, default=None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    source_path = args.source_review_report.expanduser()
    review_report = load_json(source_path)
    decision_file = load_json(args.decision_file.expanduser()) if args.decision_file else None
    report = build_visual_retrieval_hint_candidate_store_expansion_human_product_decision_record(
        review_report,
        decision_file=decision_file,
        source_review_report_ref=sanitized_report_ref(source_path, project_root=PROJECT_ROOT),
        decision_file_ref=(
            sanitized_report_ref(args.decision_file.expanduser(), project_root=PROJECT_ROOT)
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "visual retrieval hint candidate-store decision record schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write:
        paths = write_visual_retrieval_hint_candidate_store_expansion_human_product_decision_record(
            report,
