#!/usr/bin/env python3
"""Validate manually supplied expansion visual annotation web/VLM output."""
...
DEFAULT_VALIDATION_MD_PATH = (
    PROJECT_ROOT / "eval/knowledgeos/reports/visual_annotation_expansion_web_output_002.validation.v1.md"
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not report_validation.ok:
        raise ValueError(
            "visual annotation expansion web output validation report schema failed: "
            + "; ".join(str(error) for error in report_validation.errors)
        )

    if not args.no_write:
        paths = write_visual_annotation_expansion_web_output_validation(
            report,
