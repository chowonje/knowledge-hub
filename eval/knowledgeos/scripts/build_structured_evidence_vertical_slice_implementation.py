#!/usr/bin/env python3
"""Build Structured Evidence vertical slice implementation report."""
...
from knowledge_hub.papers.structured_evidence_vertical_slice_implementation import (
    ...
    render_structured_evidence_vertical_slice_implementation_markdown,
)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        / "eval/knowledgeos/reports/structured_evidence_vertical_slice_implementation.v1.json",
    )
    parser.add_argument(
        "--report-md",
        type=Path,
        default=PROJECT_ROOT
        / "eval/knowledgeos/reports/structured_evidence_vertical_slice_implementation.v1.md",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Write greenfield source_span / strict_evidence JSONL records when trace validation passes.",
    )
    ...
    if not schema_validation.get("ok"):
        ...
        raise ValueError(f"implementation report schema validation failed: {errors}")
    args.report_json.parent.mkdir(parents=True, exist_ok=True)
    args.report_json.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    args.report_md.write_text(
        render_structured_evidence_vertical_slice_implementation_markdown(payload),
        encoding="utf-8",
    )
