#!/usr/bin/env python3
"""Build production vector DB search-quality eval for visual retrieval hints."""
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/"
    "limited_visual_retrieval_hint_production_vector_db_search_quality_eval_005.v1.md"
)
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--source-layout-candidate-report", type=Path, default=DEFAULT_LAYOUT_REPORT_PATH)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_limited_visual_retrieval_hint_production_vector_db_search_quality_eval(
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "limited visual retrieval hint production vector DB search-quality eval schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write:
        paths = write_limited_visual_retrieval_hint_production_vector_db_search_quality_eval(
            report,
