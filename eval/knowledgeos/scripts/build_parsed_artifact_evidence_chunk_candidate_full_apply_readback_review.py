#!/usr/bin/env python3
"""Build parsed-artifact evidence chunk candidate full-apply readback review."""
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/parsed_artifact_evidence_chunk_candidate_full_apply_readback_review.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--full-apply-report", type=Path, default=DEFAULT_FULL_APPLY_PATH)
...
def main(argv: list[str] | None = None) -> int:
    ...
    full_apply_path = args.full_apply_report.expanduser()
    report = build_parsed_artifact_evidence_chunk_candidate_full_apply_readback_review(
        full_apply_report=load_json(full_apply_path),
        source_full_apply_report_ref=sanitized_report_ref(full_apply_path, project_root=PROJECT_ROOT),
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "parsed artifact evidence chunk candidate full-apply readback review schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write_report:
        paths = write_parsed_artifact_evidence_chunk_candidate_full_apply_readback_review(
            report,
