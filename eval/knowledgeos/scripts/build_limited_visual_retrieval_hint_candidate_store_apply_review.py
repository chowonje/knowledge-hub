#!/usr/bin/env python3
"""Build a report-only review for limited visual retrieval-hint apply design."""
...
DEFAULT_JSON_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/limited_visual_retrieval_hint_candidate_store_apply_review.v1.json"
)
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/limited_visual_retrieval_hint_candidate_store_apply_review.v1.md"
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "limited visual retrieval hint candidate-store apply review schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write:
        paths = write_limited_visual_retrieval_hint_candidate_store_apply_review(
            report,
