#!/usr/bin/env python3
"""Build the report-only visual/layout candidate list report."""
...
DEFAULT_JSON_PATH = PROJECT_ROOT / "eval/knowledgeos/reports/visual_layout_candidate_list_report.v1.json"
DEFAULT_MD_PATH = PROJECT_ROOT / "eval/knowledgeos/reports/visual_layout_candidate_list_report.v1.md"
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_visual_layout_candidate_list_report(
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "visual/layout candidate list report schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write:
        paths = write_visual_layout_candidate_list_reports(
            report,
