#!/usr/bin/env python3
"""Build final merge review for the visual retrieval-hint candidate-discovery tranche."""
...
DEFAULT_SOURCE_ROUTE_DESIGN_PATH = (
    PROJECT_ROOT
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/"
    "limited_visual_retrieval_hint_final_merge_review_005.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--source-route-design", type=Path, default=DEFAULT_SOURCE_ROUTE_DESIGN_PATH)
...
def main(argv: list[str] | None = None) -> int:
    ...
    source_route_path = args.source_route_design.expanduser()
    source_search_path = args.source_search_quality.expanduser()
    source_apply_path = args.source_apply_executor.expanduser()
    report = build_limited_visual_retrieval_hint_final_merge_review(
        runtime_candidate_discovery_route_design=load_json(source_route_path),
        ...
        source_runtime_candidate_discovery_route_design_ref=sanitized_report_ref(
            source_route_path,
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "limited visual retrieval hint final merge review schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write:
        paths = write_limited_visual_retrieval_hint_final_merge_review(
            report,
