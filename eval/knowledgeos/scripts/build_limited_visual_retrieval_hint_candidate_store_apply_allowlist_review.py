#!/usr/bin/env python3
"""Build a report-only allowlist/holdout review for visual retrieval hints."""
...
DEFAULT_APPLY_DESIGN_REPORT_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/limited_visual_retrieval_hint_candidate_store_apply_design.v1.json"
)
DEFAULT_JSON_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/limited_visual_retrieval_hint_candidate_store_apply_allowlist_review.v1.json"
)
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/limited_visual_retrieval_hint_candidate_store_apply_allowlist_review.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--source-apply-design-report", type=Path, default=DEFAULT_APPLY_DESIGN_REPORT_PATH)
...
def main(argv: list[str] | None = None) -> int:
    ...
    source_path = args.source_apply_design_report.expanduser()
    apply_design_report = load_json(source_path)
    report = build_limited_visual_retrieval_hint_candidate_store_apply_allowlist_review(
        apply_design_report,
        source_apply_design_report_ref=sanitized_report_ref(source_path, project_root=PROJECT_ROOT),
    )
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "limited visual retrieval hint candidate-store apply allowlist review schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write:
        paths = write_limited_visual_retrieval_hint_candidate_store_apply_allowlist_review(
            report,
