#!/usr/bin/env python3
"""Build the public/default promotion decision gate for the v0.1 evidence chunk RC."""
...
from knowledge_hub.papers.parsed_artifact_evidence_chunk_answer_path_v01_rc_public_default_promotion_decision_gate import (
    DEFAULT_POST_MERGE_CONVERGENCE_REPORT,
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/parsed_artifact_evidence_chunk_answer_path_v01_rc_public_default_promotion_decision_gate.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--post-merge-convergence-report", type=Path, default=DEFAULT_POST_MERGE_CONVERGENCE_REPORT)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_parsed_artifact_evidence_chunk_answer_path_v01_rc_public_default_promotion_decision_gate(
        post_merge_convergence_report_path=args.post_merge_convergence_report,
    )
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "parsed artifact evidence chunk v0.1 RC public/default promotion decision gate schema validation failed: "
            ...
            json.dumps(
                {
                    ...
                    "promotionDecision": report.get("promotionDecision"),
