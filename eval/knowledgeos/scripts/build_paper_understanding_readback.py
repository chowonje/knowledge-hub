...
DEFAULT_JSON_PATH = PROJECT_ROOT / "eval/knowledgeos/reports/paper_understanding_readback.v1.json"


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build report-only Paper Understanding readback rows from sanitized packet input."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    validation = validate_payload(report, PAPER_UNDERSTANDING_READBACK_SCHEMA_ID, strict=True)
    if not validation.ok:
        raise RuntimeError("paper understanding readback schema validation failed: " + "; ".join(validation.errors))
    paths = {} if args.no_write_report else write_paper_understanding_readback(report, report_json=args.report_json)
    if args.json:
