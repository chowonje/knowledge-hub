#!/usr/bin/env python3
"""Build the KnowledgeOS v0.1 RC corpus-scale answer quality live runner design."""
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT
    / "eval/knowledgeos/reports/knowledgeos_v01_rc_corpus_scale_answer_quality_live_runner_design.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--corpus-scale-gate-report", type=Path, default=DEFAULT_CORPUS_SCALE_GATE_REPORT)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_knowledgeos_v01_rc_corpus_scale_answer_quality_live_runner_design(
        corpus_scale_gate_report_path=args.corpus_scale_gate_report,
    )
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "KnowledgeOS v0.1 RC corpus-scale answer quality live runner design schema validation failed: "
