#!/usr/bin/env python
"""Build the v0.1 text-evidence RC convergence board."""
...
DEFAULT_JSON_PATH = REPORTS_ROOT / "text_evidence_rc_convergence.v1.json"
DEFAULT_MD_PATH = REPORTS_ROOT / "text_evidence_rc_convergence.v1.md"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--no-pr-state", action="store_true")
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_text_evidence_rc_convergence_report(
        ...
        include_pr_state=not args.no_pr_state,
    )
    if not args.no_write:
        write_report(report, json_path=args.out, markdown_path=args.md_out)
    if args.json:
        print(json.dumps(report, ensure_ascii=False, indent=2))
    return 0 if report.get("status") == "ready_for_integration_review" else 1
