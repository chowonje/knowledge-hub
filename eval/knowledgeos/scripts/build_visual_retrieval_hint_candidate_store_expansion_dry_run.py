#!/usr/bin/env python3
"""Build the report-only visual retrieval hint candidate-store expansion dry-run."""
...
DEFAULT_MD_PATH = (
    PROJECT_ROOT / "eval/knowledgeos/reports/visual_retrieval_hint_candidate_store_expansion_dry_run.v1.md"
)


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--source-design-report", type=Path, default=DEFAULT_SOURCE_DESIGN_PATH)
...
def main(argv: list[str] | None = None) -> int:
    ...
    source_path = args.source_design_report.expanduser()
    design_report = load_json(source_path)
    report = build_visual_retrieval_hint_candidate_store_expansion_dry_run(
        design_report,
        source_design_report_ref=sanitized_report_ref(source_path, project_root=PROJECT_ROOT),
    )
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "visual retrieval hint candidate-store dry-run schema validation failed: "
            + "; ".join(str(error) for error in validation.errors)
        )
    if not args.no_write:
        paths = write_visual_retrieval_hint_candidate_store_expansion_dry_run(
            report,
