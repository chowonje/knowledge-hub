#!/usr/bin/env python3
"""Build parsed-artifact evidence chunk answer-path opt-in route review."""
...
from knowledge_hub.papers.parsed_artifact_evidence_chunk_answer_path_opt_in_route_review import (
    DEFAULT_ANSWER_QUALITY_SMOKE_REPORT,
...
DEFAULT_JSON_PATH = PROJECT_ROOT / "eval/knowledgeos/reports/parsed_artifact_evidence_chunk_answer_path_opt_in_route_review.v1.json"
DEFAULT_MD_PATH = PROJECT_ROOT / "eval/knowledgeos/reports/parsed_artifact_evidence_chunk_answer_path_opt_in_route_review.v1.md"


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--answer-quality-smoke-report", type=Path, default=PROJECT_ROOT / DEFAULT_ANSWER_QUALITY_SMOKE_REPORT)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_parsed_artifact_evidence_chunk_answer_path_opt_in_route_review(
        answer_quality_smoke_report=args.answer_quality_smoke_report,
    )
    ...
    if not validation.ok:
        ...
        raise ValueError(
            "parsed artifact evidence chunk answer path opt-in route review schema validation failed: "
