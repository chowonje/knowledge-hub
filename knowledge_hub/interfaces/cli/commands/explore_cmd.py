...
from knowledge_hub.papers.source_guard import stage_source_guard
...
def citations(ctx, paper_id, limit, ingest):
    ...
    \b
    예시:
      khub explore citations 2005.14165
...
def network(paper_id, depth, citations_limit, references_limit, as_json):
    ...
    \b
    예시:
      khub explore network 2005.14165
      khub explore network 2005.14165 -d 2
      khub explore network 1706.03762 --json
    """
...
def _ingest_papers_to_sqlite(ctx, papers):
    ...
    try:
        ...
        for p in papers:
            ...
            if existing:
                continue
            guard = stage_source_guard(sqlite_db, paper_id=p.arxiv_id, title=p.title)
            if str(guard.get("status") or "") == "suspected":
                suspected.append(
                    {
                        "paperId": str(p.arxiv_id or ""),
        ...
        if suspected:
            console.print(f"[yellow]source guard pending[/yellow]: {len(suspected)}편 title collision candidate 감지")
            for item in suspected[:5]:
                console.print(f"[yellow]- {item['paperId']} {item['title'][:60]}[/yellow]")
        if added > 0:
            console.print("[dim]전체 파이프라인: khub discover 또는 khub paper download/summarize/embed[/dim]")
        return {"added": added, "suspected": suspected}
    except Exception as e:
        console.print(f"[red]저장 실패: {e}[/red]")
        return {"added": 0, "suspected": [], "error": str(e)}
