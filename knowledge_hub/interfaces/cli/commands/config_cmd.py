...
def config_list(ctx):
    ...
    sections = [
        ...
        ("paper.summary", ["parser"]),
        ("paper.summary.opendataloader", ["reading_order", "use_struct_tree", "table_method"]),
        ("routing.llm.hybrid", ["enabled", "local.provider", "local.model", "api.provider", "api.model"]),
        ("storage", ["papers_dir", "vector_db", "sqlite"]),
        ("indexing", ["failure_report_dir", "embed_batch_size", "embed_pause_ms"]),
        ("obsidian", ["enabled", "vault_path", "vault_name", "notes_folder", "write_backend", "cli_binary"]),
    ]
    for section, keys in sections:
        for i, key in enumerate(keys):
            key_path = [token for token in (section.split(".") + key.split(".")) if token]
            value = config.get_nested(*key_path, default="(미설정)")
...
def config_providers(ctx, models):
    ...
    console.print("[dim]PPLX-Local 엔드포인트: khub config set providers.pplx-local.base_url <URL>[/dim]")
    console.print("[dim]PPLX-ST 배치크기: khub config set providers.pplx-st.batch_size 8[/dim]")
    console.print("[dim]PPLX-ST 디바이스: khub config set providers.pplx-st.device mps[/dim]")
    console.print("[dim]PPLX-ST 스레드: khub config set providers.pplx-st.torch_num_threads 1[/dim]")
    console.print("[dim]PPLX-ST 청크크기: khub config set providers.pplx-st.max_chars_per_chunk 1000[/dim]")
