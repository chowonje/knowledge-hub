...
def graph_concept(ctx, name):
    ...
    concept = db.resolve_entity(name, entity_type="concept")
    if not concept:
        ...
        if not concept:
            ...
            return
    cid = str(concept.get("entity_id", "")).strip()
    canonical = str(concept.get("canonical_name", "")).strip() or name
...
def graph_path(ctx, source, target, max_depth):
    ...
    src_entity = db.resolve_entity(source, entity_type="concept")
    tgt_entity = db.resolve_entity(target, entity_type="concept")
    ...
    src_id = str(src_entity.get("entity_id", "")).strip()
    tgt_id = str(tgt_entity.get("entity_id", "")).strip()
    src_canonical = str(src_entity.get("canonical_name", source))
    tgt_canonical = str(tgt_entity.get("canonical_name", target))
    ...
    for nid in found_path:
        c = db.get_ontology_entity(nid)
        id_to_name[nid] = str(c.get("canonical_name", nid)) if c else nid
