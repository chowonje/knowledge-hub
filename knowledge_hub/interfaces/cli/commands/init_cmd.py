...
def _repo_root_from(module_file: str | Path) -> Path:
...
def _dotenv_candidates(cwd: Path | None = None, module_file: str | Path | None = None) -> list[Path]:
    current_dir = (cwd or Path.cwd()).resolve()
    repo_root = _repo_root_from(module_file or __file__)
    candidates = [current_dir / ".env", repo_root / ".env"]
    deduped: list[Path] = []
    seen: set[Path] = set()
...
def _load_dotenv(cwd: Path | None = None, module_file: str | Path | None = None):
    """프로젝트 .env 파일에서 환경 변수 로드"""
    try:
        from dotenv import load_dotenv

        for env_path in _dotenv_candidates(cwd=cwd, module_file=module_file):
            if env_path.exists():
...
def init_cmd(ctx, non_interactive):
    ...
    config = Config()
    prompt_defaults = get_public_prompt_defaults()
    ...
    trans_provider, trans_model = _prompt_provider(
        "번역(Translation)",
        default=str((prompt_defaults.get("translation") or {}).get("provider") or "openai"),
    )
    config.set_nested("translation", "provider", trans_provider)
    trans_model_input = click.prompt(
        "  번역 모델",
        default=trans_model or str((prompt_defaults.get("translation") or {}).get("model") or "gpt-5-nano"),
    )
    config.set_nested("translation", "model", trans_model_input)

    summ_provider, summ_model = _prompt_provider(
        "요약(Summarization)",
        default=str((prompt_defaults.get("summarization") or {}).get("provider") or "ollama"),
    )
    config.set_nested("summarization", "provider", summ_provider)
    summ_model_input = click.prompt(
        "  요약 모델",
        default=summ_model or str((prompt_defaults.get("summarization") or {}).get("model") or "qwen3:14b"),
    )
    config.set_nested("summarization", "model", summ_model_input)

    embed_provider, embed_model = _prompt_embed_provider(
        default=str((prompt_defaults.get("embedding") or {}).get("provider") or "ollama")
    )
    config.set_nested("embedding", "provider", embed_provider)
    embed_model_input = click.prompt(
        "  임베딩 모델",
        default=embed_model or str((prompt_defaults.get("embedding") or {}).get("model") or "nomic-embed-text"),
    )
