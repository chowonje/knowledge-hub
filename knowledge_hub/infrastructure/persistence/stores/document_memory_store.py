...
class DocumentMemoryStore:
    ...
    def replace_units(self, *, document_id: str, units: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        normalized_units = [dict(unit or {}) for unit in units or []]
        doc_hash = document_source_hash(token, normalized_units)
        self.conn.execute("DELETE FROM document_memory_units WHERE document_id = ?", (token,))
        for payload in normalized_units:
            unit_source_hash = str(payload.get("source_content_hash") or payload.get("content_hash") or doc_hash or "").strip()
            self.conn.execute(
                """
        ...
        mark_derivatives_stale_for_document(self.conn, document_id=token, source_content_hash=doc_hash)
        self.conn.commit()
        return self.list_document_units(token, limit=max(1, len(normalized_units) + 4))
    ...
    def search_units(
        ...
        *,
        ...
        where_parts: list[str] = [] if include_stale else ["COALESCE(stale, 0) = 0"]
