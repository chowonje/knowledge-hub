"""Post-apply promotion hold review for StrictEvidence citation-grade rows.

Consumes the citation-grade apply readback review report and documents that the
citation-grade set remains candidate-only before any runtime, parser, answer, DB,
...
STRICT_EVIDENCE_CITATION_GRADE_POST_APPLY_PROMOTION_HOLD_REVIEW_SCHEMA_ID = (
    "knowledge-hub.paper.strict-evidence-citation-grade-post-apply-promotion-hold-review.v1"
)

HOLD_STATUS_ACTIVE = "citation_grade_post_apply_promotion_hold_active"
...
DEFAULT_READBACK_REVIEW_REPORT_PATH = (
    ...
    / "strict-evidence-citation-grade-executor-apply-readback-review.json"
)
...
_DOWNSTREAM_GATE_FIELDS = {
    ...
    "parentCitationGradeBoolean": ("citationReady", "citationGradeAllowed", "citationGrade"),
...
def _future_promotion_readiness_checklist() -> list[dict[str, Any]]:
    return [
        {
            "id": "citation_grade_apply_readback_review",
            "title": "Citation-grade apply readback review for all applied rows",
            "status": "completed",
        ...
        },
        {
            ...
            "title": "Rollback strategy for citation-grade/runtime promotion records",
            "status": "pending",
            "requiredBeforePromotion": True,
        },
        {
            "id": "parent_citation_grade_boolean_mutation_change_requires_new_decision",
            "title": "Any parent citationGrade boolean mutation requires a new ADR",
            "status": "blocked",
            "requiredBeforePromotion": True,
        },
        {
...
def _input_report_violations(
    *,
    ...
    expectations = {
        ...
        "readbackValidatedRows": expected_citation_grade_record_rows,
    }
    ...
    *,
    ...
    for index, readback_row in enumerate(readback_rows):
        ...
        if schema_violations:
            ...
            or not _safe_bool(source_row.get("citationGradeReadbackValidated"))
        ):
        ...
        rows.append(
            {
                "hold_row_id": (
                    f"strict-evidence-citation-grade-post-apply-promotion-hold-review:{index:04d}"
                ),
...
def _count_rows(
    *,
    ...
    return {
        ...
        "citationGradeRecordRows": _safe_int(readback_counts.get("citationGradeRecordRows")),
        "readbackValidatedRows": _safe_int(readback_counts.get("readbackValidatedRows")),
        "citationGradeHoldRows": hold_active_count,
...
def build_strict_evidence_citation_grade_post_apply_promotion_hold_review(
    *,
    ...
    citation_grade_hold_rows = int(counts.get("citationGradeHoldRows") or 0)
    status = "ok"
    if (
        aggregate_violations
        or not rows
        or citation_grade_hold_rows != expected_citation_grade_record_rows
        or citation_grade_hold_rows != len(rows)
    ...
    return {
        "schema": STRICT_EVIDENCE_CITATION_GRADE_POST_APPLY_PROMOTION_HOLD_REVIEW_SCHEMA_ID,
        "status": status,
        ...
        "gate": {
            "citationGradePostApplyPromotionHoldReviewReady": status == "ok",
            "holdDecision": (
                "strict_evidence_citation_grade_post_apply_promotion_hold_active"
                if status == "ok"
                else "strict_evidence_citation_grade_post_apply_promotion_hold_blocked"
            ),
            "citationGradeRecordWriteAllowed": False,
            "citationGradeBooleanMutationAllowed": False,
            "eligibilityRecordWriteAllowed": False,
            ...
            "recommendedNextTranche": (
                "strict_evidence_citation_grade_runtime_binding_gate_design"
                if status == "ok"
                else "strict_evidence_citation_grade_executor_apply_readback_repair"
            ),
        },
...
def render_strict_evidence_citation_grade_post_apply_promotion_hold_review_markdown(
    report: dict[str, Any],
) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- citation-grade hold rows: {int(counts.get('citationGradeHoldRows') or 0)}",
...
def write_strict_evidence_citation_grade_post_apply_promotion_hold_review_reports(
    report: dict[str, Any],
    output_dir: str | Path,
) -> dict[str, str]:
    root = Path(str(output_dir)).expanduser()
    root.mkdir(parents=True, exist_ok=True)
    report_path = root / "strict-evidence-citation-grade-post-apply-promotion-hold-review.json"
    summary_path = root / "strict-evidence-citation-grade-post-apply-promotion-hold-review-summary.json"
    markdown_path = root / "strict-evidence-citation-grade-post-apply-promotion-hold-review.md"
    ...
    markdown_path.write_text(
        render_strict_evidence_citation_grade_post_apply_promotion_hold_review_markdown(report),
        encoding="utf-8",
    )
...
def main(argv: list[str] | None = None) -> int:  # pragma: no cover
    parser = ArgumentParser(
        description=(
            "Review post-apply promotion hold state for StrictEvidence citation-grade rows "
    ...
    parser.add_argument(
        ...
        help="Path to citation-grade executor apply readback review JSON report.",
    )
    ...
    report = build_strict_evidence_citation_grade_post_apply_promotion_hold_review(
        readback_review_report_path=args.readback_report,
        paper_ids=args.paper_id or None,
    )
    paths = write_strict_evidence_citation_grade_post_apply_promotion_hold_review_reports(
        report,
        args.output_dir,
    )
...
__all__ = [
    ...
    "render_strict_evidence_citation_grade_post_apply_promotion_hold_review_markdown",
