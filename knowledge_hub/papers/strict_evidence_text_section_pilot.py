"""Report-only text-section pilot for parsed-artifact StrictEvidence promotion.

Consumes the promotion tranche-plan report, validates section-assigned rows against
stored StrictEvidence authority contracts, and holds out non-section tranches without
mutating records or enabling integration surfaces.
"""
...
STRICT_EVIDENCE_TEXT_SECTION_PILOT_SCHEMA_ID = (
    "knowledge-hub.paper.strict-evidence-text-section-pilot.v1"
)

PILOT_STATUS_CANDIDATE_ONLY = "text_section_pilot_candidate_only"
PILOT_STATUS_HELD_OUT_NON_SECTION = "held_out_non_section_tranche"
...
def _classify_section_pilot_row(
    plan_row: dict[str, Any],
    *,
    strict_evidence_record: dict[str, Any],
) -> tuple[str, list[str]]:
    ...
    if planned_tranche != TRANCHE_TEXT_SECTION_PILOT:
        if artifact_type == "figure" or planned_tranche == TRANCHE_FIGURE_CAPTION_PILOT:
            return PILOT_STATUS_HELD_OUT_NON_SECTION, []
...
def _pilot_rows(plan_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, plan_row in enumerate(plan_rows):
        ...
        pilot_status, blockers = _classify_section_pilot_row(
            source_row,
            strict_evidence_record=strict_evidence_record,
        )
        ...
        rows.append(
            {
                ...
                "textSectionPilotCandidateOnly": ready,
                ...
                "recommended_action": (
                    "queue_for_strict_evidence_text_section_pilot_executor_dry_run"
                    if ready
                    else (
                        "held_for_figure_caption_pilot_tranche"
                        if pilot_status == PILOT_STATUS_HELD_OUT_NON_SECTION
                        else "repair_tranche_plan_or_strict_evidence_before_section_pilot"
                    )
                ),
            }
        )
...
def _held_out_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    held_rows = [row for row in rows if row.get("pilot_status") == PILOT_STATUS_HELD_OUT_NON_SECTION]
    return {
...
def _count_rows(
    *,
    ...
    section_pilot_input_rows: int,
) -> dict[str, Any]:
    section_rows = [
        row for row in rows if _safe_text(row.get("planned_tranche")) == TRANCHE_TEXT_SECTION_PILOT
    ]
    return {
        "inputRows": len(rows),
        "sectionPilotInputRows": len(section_rows),
        "textSectionPilotCandidateOnlyRows": sum(
            1 for row in rows if row.get("pilot_status") == PILOT_STATUS_CANDIDATE_ONLY
        ),
        "heldOutNonSectionRows": sum(
            1 for row in rows if row.get("pilot_status") == PILOT_STATUS_HELD_OUT_NON_SECTION
        ),
        ...
        "sectionPilotInputRowsFromTranchePlan": section_pilot_input_rows,
    }


def build_strict_evidence_text_section_pilot(
    *,
    ...
    section_pilot_input_rows = int(
        (tranche_plan_report.get("counts") or {}).get("sectionPilotRows")
        or sum(
            1
            for row in all_plan_rows
            if _safe_text(row.get("planned_tranche")) == TRANCHE_TEXT_SECTION_PILOT
        )
    ...
    if input_schema_violations:
        for row in rows:
            if row.get("pilot_status") == PILOT_STATUS_HELD_OUT_NON_SECTION:
                continue
            ...
            row["textSectionPilotCandidateOnly"] = False
            row["recommended_action"] = "repair_tranche_plan_report_schema_before_section_pilot"

    counts = _count_rows(
        rows=rows,
        input_schema_violations=_dedupe(input_schema_violations),
        section_pilot_input_rows=section_pilot_input_rows,
    )
    candidate_rows = int(counts.get("textSectionPilotCandidateOnlyRows") or 0)
    held_out_rows = int(counts.get("heldOutNonSectionRows") or 0)
    status = "ok"
    if (
        input_schema_violations
        or not rows
        or candidate_rows != section_pilot_input_rows
    ...
    return {
        "schema": STRICT_EVIDENCE_TEXT_SECTION_PILOT_SCHEMA_ID,
        "status": status,
        ...
        "gate": {
            "textSectionPilotReady": (
                bool(candidate_rows)
                and candidate_rows == section_pilot_input_rows
            ...
            "figureRowsProcessedAsActivePilot": False,
            "schemaViolations": _dedupe(input_schema_violations),
            "decision": (
                "strict_evidence_text_section_pilot_ready"
                if status == "ok"
                else "blocked"
            ),
            "recommendedNextTranche": (
                "strict_evidence_text_section_pilot_executor_dry_run"
                if status == "ok"
...
def render_strict_evidence_text_section_pilot_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- section pilot input rows: {int(counts.get('sectionPilotInputRows') or 0)}",
            f"- text section pilot candidate-only rows: {int(counts.get('textSectionPilotCandidateOnlyRows') or 0)}",
...
def write_strict_evidence_text_section_pilot_reports(
    report: dict[str, Any],
    output_dir: str | Path,
) -> dict[str, str]:
    root = Path(str(output_dir)).expanduser()
    root.mkdir(parents=True, exist_ok=True)
    report_path = root / "strict-evidence-text-section-pilot.json"
    summary_path = root / "strict-evidence-text-section-pilot-summary.json"
    markdown_path = root / "strict-evidence-text-section-pilot.md"
    ...
    markdown_path.write_text(
        render_strict_evidence_text_section_pilot_markdown(report),
        encoding="utf-8",
    )
...
def main(argv: list[str] | None = None) -> int:  # pragma: no cover
    parser = ArgumentParser(
        description=(
            "Plan the text-section StrictEvidence pilot from tranche-plan rows without "
    ...
    report = build_strict_evidence_text_section_pilot(
        tranche_plan_report_path=args.tranche_plan_report,
        paper_ids=args.paper_id or None,
    )

    if args.output_dir:
        paths = write_strict_evidence_text_section_pilot_reports(report, args.output_dir)
...
__all__ = [
    ...
    "TRANCHE_TEXT_SECTION_PILOT",
    "build_strict_evidence_text_section_pilot",
    "render_strict_evidence_text_section_pilot_markdown",
    "write_strict_evidence_text_section_pilot_reports",
]
