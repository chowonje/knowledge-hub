"""Apply-gated executor for StrictEvidence citation-grade JSONL records.

Consumes the citation-grade executor dry-run report, validates dry-run-ready
rows, and appends citation-grade records only when explicitly invoked with
``--apply`` and ``--papers-dir``. Parent StrictEvidence, eligibility, and
SourceSpan stores remain read-only.
"""
...
STRICT_EVIDENCE_CITATION_GRADE_EXECUTOR_APPLY_SCHEMA_ID = (
    "knowledge-hub.paper.strict-evidence-citation-grade-executor-apply.v1"
)

APPLY_STATUS_READY = "apply_ready_citation_grade_record_only"
APPLY_STATUS_APPLIED = "applied_citation_grade_record"
...
APPLY_STATUS_BLOCKED_STORE_CONTRACT = "blocked_citation_grade_store_contract_not_ready"
APPLY_STATUS_BLOCKED_SCHEMA_VIOLATION = "blocked_citation_grade_record_schema_violation"
...
DEFAULT_EXECUTOR_DRY_RUN_REPORT_PATH = (
    ...
    / "strict-evidence-citation-grade-executor-dry-run.json"
)
...
DEFAULT_REPORT_ROOT = (
    ...
    / "layout-parser-pilot"
    / "2026-05-20"
    / "strict-evidence-citation-grade-executor-apply"
)

DEFAULT_DRY_RUN_OUTPUT_DIR = (
    DEFAULT_REPORT_ROOT / "01-strict-evidence-citation-grade-executor-apply-dry-run"
)
DEFAULT_APPLY_OUTPUT_DIR = (
    DEFAULT_REPORT_ROOT / "02-strict-evidence-citation-grade-executor-apply"
)

DEFAULT_OUTPUT_DIR = DEFAULT_DRY_RUN_OUTPUT_DIR

ROLLBACK_STRATEGY = CITATION_GRADE_STORE_CONTRACT.get(
    "rollbackStrategy",
    "delete citation-grade records written by the explicit run_id only",
)
...
def _citation_grade_only_policy_matrix() -> dict[str, Any]:
    return {
...
def _contract_ready(contract_payload: dict[str, Any]) -> tuple[bool, list[str]]:
    blockers: list[str] = []
    if _safe_text(contract_payload.get("schema")) != STRICT_EVIDENCE_CITATION_GRADE_RECORD_CONTRACT_SCHEMA_ID:
        blockers.append("citation_grade_contract_schema_mismatch")
    if _safe_text(contract_payload.get("status")) != "ok":
        blockers.append(
            f"citation_grade_contract_status={_safe_text(contract_payload.get('status')) or 'unknown'}"
        )
    write_targets = contract_payload.get("writeTargets")
    if not isinstance(write_targets, list) or not write_targets:
        blockers.append("citation_grade_contract_write_targets_missing")
    else:
        target = write_targets[0] if isinstance(write_targets[0], dict) else {}
        if _safe_text(target.get("plannedWriteTarget")) != STRICT_EVIDENCE_CITATION_GRADE_STORE:
            blockers.append("citation_grade_contract_write_target_mismatch")
    gate = contract_payload.get("gate") if isinstance(contract_payload.get("gate"), dict) else {}
    if _safe_text(gate.get("decision")) != "strict_evidence_citation_grade_record_contract_ready":
        blockers.append(
            f"citation_grade_contract_decision={_safe_text(gate.get('decision')) or 'unknown'}"
        )
    return not blockers, blockers


def _planned_record(dry_row: dict[str, Any]) -> dict[str, Any]:
    planned = dry_row.get("plannedCitationGradeRecord")
...
def _row_is_apply_ready(dry_row: dict[str, Any]) -> tuple[bool, str, list[str]]:
    ...
    if not _safe_bool(dry_row.get("dryRunReadyCitationGradeRecordOnly")):
        blockers.append("dryRunReadyCitationGradeRecordOnly_false")

    if _safe_text(dry_row.get("recommended_action")) != "queue_for_citation_grade_executor_apply_dry_run_review":
        blockers.append(
    ...
    if not planned:
        blockers.append("plannedCitationGradeRecord_missing")

    if _safe_text(planned.get("plannedWriteTarget")) != STRICT_EVIDENCE_CITATION_GRADE_STORE:
        blockers.append(
    ...
    if not _safe_text(planned.get("citationGradeRecordId")):
        blockers.append("citationGradeRecordId_missing")
    ...
    if blockers:
        ...
        if any(
            ...
            or item == "dryRunReadyCitationGradeRecordOnly_false"
...
def _citation_grade_record(dry_row: dict[str, Any], *, run_id: str) -> dict[str, Any]:
    ...
    record["citationGradeBooleanMutationAllowed"] = False
    record["citationGradeMutationApplied"] = False
...
def _write_jsonl_idempotent(path: Path, records: list[dict[str, Any]]) -> int:
    ...
    incoming_by_key = {
        str(record.get("idempotencyKey") or record.get("citationGradeRecordId")): record
        for record in records
    }
    retained: list[dict[str, Any]] = []
    for existing in _read_jsonl(path):
        key = str(existing.get("idempotencyKey") or existing.get("citationGradeRecordId"))
    ...
    *,
    ...
    for path, path_records in sorted(records_by_path.items(), key=lambda item: str(item[0])):
        ...
        for record in path_records:
            ...
            if stored == record:
                readback_rows += 1
                path_by_record_id[_safe_text(record.get("citationGradeRecordId"))] = str(path)
            else:
                warnings.append(f"readback_mismatch:{record.get('citationGradeRecordId')}")
    return applied_rows, readback_rows, _dedupe(warnings), path_by_record_id


def build_strict_evidence_citation_grade_executor_apply(
    *,
    executor_dry_run_report_path: str | Path = DEFAULT_EXECUTOR_DRY_RUN_REPORT_PATH,
    citation_grade_record_contract_report_path: str | Path = DEFAULT_CONTRACT_REPORT_PATH,
    ...
    run_id = _safe_text(run_id) or f"strict-evidence-citation-grade-executor-apply-{_now_iso()}"
    ...
    if not contract_payload:
        schema_violations.append("citation_grade_contract_report_missing_or_unreadable")
    ...
    if dry_run_payload:
        ...
        if not _safe_bool(gate.get("readyForCitationGradeExecutorDryRun")):
            schema_violations.append("executor_dry_run_not_ready_for_apply")

    dry_run_ready_rows = (
        int(
            (dry_run_payload.get("counts") or {}).get("dryRunReadyCitationGradeRecordOnlyRows")
            or 0
        )
        if dry_run_payload
        else 0
    ...
    citation_grade_records: list[dict[str, Any]] = []
    rows: list[dict[str, Any]] = []
    for index, dry_row in enumerate(input_rows):
        ...
        citation_grade_record: dict[str, Any] | None = None
        citation_grade_record_id = ""
        citation_grade_store_path = ""
        if ready:
            citation_grade_record = _citation_grade_record(dry_row, run_id=run_id)
            citation_grade_record_id = _safe_text(citation_grade_record.get("citationGradeRecordId"))
            if papers_dir:
                citation_grade_store_path = str(
                    _record_path(papers_dir, _safe_text(citation_grade_record.get("paperId")))
                )
            validation = validate_payload(
                citation_grade_record,
                STRICT_EVIDENCE_CITATION_GRADE_RECORD_SCHEMA_ID,
                strict=True,
            )
            semantic_errors = validate_citation_grade_record_semantics(citation_grade_record)
            if validation.ok and not semantic_errors:
                citation_grade_records.append(citation_grade_record)
            else:
                ...
                schema_violations.extend(
                    f"citation_grade_record_schema_violation:{citation_grade_record_id}:{error}"
                    for error in validation.errors
                )
                schema_violations.extend(
                    f"citation_grade_record_semantic_violation:{citation_grade_record_id}:{error}"
                    for error in semantic_errors
                )

        rows.append(
            {
                "apply_row_id": f"strict-evidence-citation-grade-executor-apply:{index:04d}",
                "dry_run_row_id": _safe_text(dry_row.get("dry_run_row_id")),
                "policy_design_row_id": _safe_text(dry_row.get("policy_design_row_id")),
                ...
                "idempotencyKey": _safe_text((citation_grade_record or {}).get("idempotencyKey")),
                "planned_write_target": STRICT_EVIDENCE_CITATION_GRADE_STORE,
                "citation_grade_record_schema": (
                    STRICT_EVIDENCE_CITATION_GRADE_RECORD_SCHEMA_ID if ready else ""
                ),
                "citation_grade_store_path": citation_grade_store_path,
                "would_write_citation_grade_record": ready and not apply,
                "applied_citation_grade_record": False,
                ...
                "recommended_action": (
                    "run_citation_grade_record_apply_with_explicit_apply_flag"
                    if ready and not apply
                    else (
                        "citation_grade_record_apply_recorded"
                        if ready and apply
                        else "repair_dry_run_row_before_citation_grade_executor_apply"
                    )
                ),
            }
        )
    ...
    readback_rows = 0
    path_by_record_id: dict[str, str] = {}
    if apply and citation_grade_records and not schema_violations and papers_dir:
        applied_rows, readback_rows, readback_warnings, path_by_record_id = _apply_records(
            citation_grade_records,
            papers_dir=papers_dir,
        )
        warnings.extend(readback_warnings)
        for row in rows:
            citation_grade_record_id = _safe_text(row.get("citationGradeRecordId"))
            if citation_grade_record_id in path_by_record_id:
                row["apply_status"] = APPLY_STATUS_APPLIED
                row["would_write_citation_grade_record"] = False
                row["applied_citation_grade_record"] = True
                row["readback_validated"] = True
                row["citationGradeRecordWriteRows"] = 1
                row["citation_grade_store_path"] = path_by_record_id[citation_grade_record_id]
                row["rollback_eligible"] = True

    counts = _count_rows(
        input_rows=input_rows,
        apply_rows=rows,
        citation_grade_records=citation_grade_records,
    ...
    if (
        schema_violations
        or not rows
        or len(citation_grade_records) != len(ready_input_rows)
        or len(ready_input_rows) != dry_run_ready_rows
    ):
        status = "blocked"
    elif apply and (
        applied_rows != len(citation_grade_records) or readback_rows != len(citation_grade_records)
    ):
        status = "blocked"
        schema_violations.append("apply_readback_incomplete")

    policy_matrix = _citation_grade_only_policy_matrix()

    return {
        "schema": STRICT_EVIDENCE_CITATION_GRADE_EXECUTOR_APPLY_SCHEMA_ID,
        "status": status,
        ...
        "input": {
            ...
            "citationGradeRecordContractSchema": _safe_text(contract_payload.get("schema")),
            "citationGradeRecordContractStatus": _safe_text(contract_payload.get("status")),
        ...
        "counts": counts,
        "citationGradeOnlyPolicyMatrix": policy_matrix,
        "gate": {
            "readyForDryRunApplyPlanning": status == "ok" and not apply,
            "readyForCitationGradeRecordApply": status == "ok" and bool(citation_grade_records),
            "applyMode": bool(apply),
            "citationGradeRecordWriteAllowed": bool(apply and papers_dir and not schema_violations),
            ...
            "decision": (
                "strict_evidence_citation_grade_executor_apply_ready"
                if status == "ok" and not apply
                else (
                    "strict_evidence_citation_grade_executor_applied"
                    if status == "ok" and apply
                    else "strict_evidence_citation_grade_executor_apply_blocked"
                )
            ),
            "recommendedNextTranche": (
                "strict_evidence_citation_grade_executor_apply_readback_review"
                if status == "ok" and apply
                else "strict_evidence_citation_grade_executor_apply_review"
            ),
        },
        "policy": {
            "reportOnly": not apply,
            "dryRunByDefault": True,
            "applyRequiredForCitationGradeStoreWrites": True,
            "citationGradeRecordWrite": bool(applied_rows),
            "citationGradeBooleanMutation": False,
            "eligibilityRecordWrite": False,
            "strictEvidenceStoreWrite": False,
            "sourceSpanStoreWrite": False,
        ...
        "rows": rows,
        "citationGradeRecords": citation_grade_records,
    }
...
def _count_rows(
    *,
    ...
    applied_citation_grade_rows = sum(
        1 for row in apply_rows if _safe_text(row.get("apply_status")) == APPLY_STATUS_APPLIED
    )
    return {
        "inputRows": len(input_rows),
        "dryRunReadyCitationGradeRecordRows": dry_run_ready_rows,
        "plannedApplyRows": planned_apply_rows,
        "appliedCitationGradeRecordRows": applied_citation_grade_rows,
        "heldInputRows": len(input_rows) - len(citation_grade_records),
        "citationGradeRecordRows": len(citation_grade_records),
        "citationGradeRecordWriteRows": applied_rows if apply_mode else 0,
...
def _summary_payload(report: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        for key in (
            ...
            "input",
            "counts",
            "citationGradeOnlyPolicyMatrix",
            "gate",
            "policy",
            "warnings",
            "rows",
            "citationGradeRecords",
        )
        if key in report
    }


def render_strict_evidence_citation_grade_executor_apply_markdown(
    report: dict[str, Any],
) -> str:
    counts = dict(report.get("counts") or {})
    matrix = dict(report.get("citationGradeOnlyPolicyMatrix") or {})
    by_status = [
        f"{status}: {count}"
    ...
    return "\n".join(
        [
            ...
            f"- applied citation-grade record rows: {int(counts.get('appliedCitationGradeRecordRows') or 0)}",
            ...
            "",
            "## Citation-grade-only policy matrix",
            f"- planned write target: {matrix.get('plannedWriteTarget', '')}",
            f"- citation-grade record write: {json.dumps(matrix.get('citationGradeRecordWrite'))}",
            f"- citationGrade boolean mutation: {json.dumps(matrix.get('citationGradeBooleanMutation'))}",
            "",
...
def write_strict_evidence_citation_grade_executor_apply_reports(
    report: dict[str, Any],
    output_dir: str | Path,
) -> dict[str, str]:
    root = Path(str(output_dir)).expanduser()
    root.mkdir(parents=True, exist_ok=True)
    report_path = root / "strict-evidence-citation-grade-executor-apply.json"
    summary_path = root / "strict-evidence-citation-grade-executor-apply-summary.json"
    markdown_path = root / "strict-evidence-citation-grade-executor-apply.md"
    ...
    markdown_path.write_text(
        render_strict_evidence_citation_grade_executor_apply_markdown(report),
        encoding="utf-8",
    )
...
def main(argv: list[str] | None = None) -> int:  # pragma: no cover
    parser = ArgumentParser(
        description=(
            "Dry-run or explicitly apply StrictEvidence citation-grade JSONL records "
            "from a citation-grade executor dry-run report without mutating parent stores."
        )
    )
    parser.add_argument(
        "--executor-dry-run-report",
        default=str(DEFAULT_EXECUTOR_DRY_RUN_REPORT_PATH),
        help="Path to citation-grade executor dry-run JSON report.",
    )
    parser.add_argument(
        "--citation-grade-record-contract-report",
        default=str(DEFAULT_CONTRACT_REPORT_PATH),
        help="Path to citation-grade record contract JSON report.",
    )
    parser.add_argument("--paper-id", action="append", default=[], help="Filter to paper id; repeatable.")
    parser.add_argument(
        "--papers-dir",
        default="",
        help="Local papers_dir root. Required with --apply for citation-grade JSONL writes.",
    )
    parser.add_argument("--run-id", default="", help="Run id recorded on citation-grade records.")
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Append citation-grade records to structured_evidence/strict_evidence_citation_grade/.",
    )
    parser.add_argument(
        "--output-dir",
        default="",
    ...
    paths = write_strict_evidence_citation_grade_executor_apply_reports(report, output_dir)
...
__all__ = [
    ...
    "default_output_dir",
    "render_strict_evidence_citation_grade_executor_apply_markdown",
    "resolve_output_dir",
