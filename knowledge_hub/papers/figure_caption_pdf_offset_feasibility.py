"""Report-only FigureCaption original PDF offset feasibility audit.

This helper checks whether FigureCaptionCandidate rows can be located in the
original local source PDF text.  Recovered offsets stay non-strict because the
figure/image region link is still unverified and runtime promotion is outside
this tranche.
"""
...
def _base_row(row: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "feasibility_row_id": f"figurecaption-pdf-offset:{row.get('paper_id')}:{row.get('candidate_id')}",
        "source_figure_caption_candidate_id": str(row.get("candidate_id") or ""),
        "paper_id": str(row.get("paper_id") or ""),
        "candidate_text": _clean_text(row.get("candidate_text")),
        "caption_text": _clean_text(row.get("caption_text")),
        "figure_label": _clean_text(row.get("figure_label")),
        ...
        "evidence_tier": "figure_caption_pdf_offset_feasibility_only",
        "report_only": True,
    ...
    *,
    ...
    return {
        **_base_row(row, context),
        ...
        "non_strict_reason": [
            "figure_caption_pdf_offset_feasibility_only",
...
def _feasibility_row(row: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    if match_result["status"] == "ambiguous":
        ...
        return _blocked_row(
            ...
            reason="original_pdf_match_ambiguous",
    ...
    if match_result["status"] != "ok":
        return _blocked_row(
            ...
            reason="unique_original_pdf_caption_match_not_found",
            attempted_matches=list(match_result.get("attempted") or []),
        )
    ...
    if str(match.get("match_target") or "") != "full_candidate_text":
        blockers.append("full_caption_label_not_recovered_in_original_pdf_match")
    return {
        **_base_row(row, context),
        ...
        "non_strict_reason": [
            "original_pdf_offset_recovered_in_report_only_feasibility_audit",
            "figure_region_link_still_unverified",
            "no_runtime_or_strict_evidence_created",
        ],
    }
...
def build_figure_caption_pdf_offset_feasibility_report(
    *,
    figure_caption_candidate_report: str | Path,
    pymupdf_parsed_root: str | Path,
    pdf_page_text_loader: Callable[[str | Path], list[dict[str, Any]]] | None = None,
) -> dict[str, Any]:
    """Build a report-only FigureCaption original PDF offset feasibility report."""

    report_path = Path(str(figure_caption_candidate_report)).expanduser()
    parsed_root = Path(str(pymupdf_parsed_root)).expanduser()
    candidate_report = _read_json(report_path)
    schema_violations: list[str] = []
    if candidate_report.get("schema") != FIGURE_CAPTION_CANDIDATE_REPORT_SCHEMA_ID:
        schema_violations.append("figure_caption_candidate_report_schema_mismatch")
    if candidate_report.get("status") not in {"ok", "empty"}:
        schema_violations.append("figure_caption_candidate_report_status_unexpected")
    ...
    return {
        ...
        "generatedAt": _now(),
        "inputs": {
            "figureCaptionCandidateReport": str(report_path),
            "figureCaptionCandidateSchema": str(candidate_report.get("schema") or ""),
        ...
        "gate": {
            ...
            "recommendedNextTranche": "review_figure_caption_offsets_and_region_links_before_any_strict_promotion",
        },
        "policy": {
            "reportOnly": True,
            "dryRunOnly": True,
        ...
        "warnings": [
            "figure_caption_pdf_offsets_are_report_only_feasibility_results",
            "recovered_caption_offsets_do_not_verify_figure_or_image_regions",
            "figure_region_link_remains_required_before_citation_grade_evidence",
...
def render_figure_caption_pdf_offset_feasibility_markdown(report: dict[str, Any]) -> str:
    counts = dict(report.get("counts") or {})
    gate = dict(report.get("gate") or {})
    lines = [
        "# FigureCaption Original PDF Offset Feasibility",
        "",
        ...
        f"- Page agreement rows: `{int(counts.get('pageAgreementRows') or 0)}`",
        f"- Figure-region verified rows: `{int(counts.get('figureRegionVerifiedRows') or 0)}`",
        f"- Strict eligible rows: `{int(counts.get('strictEligibleRows') or 0)}`",
        "",
        "## Boundary",
        "",
        "This is a report-only feasibility audit. It reads local source PDFs and records unique caption text matches, but does not verify the figure/image region, write canonical parsed artifacts, mutate SQLite, reindex, reembed, create strict evidence, allow runtime citations, or change parser routing.",
        "",
        "## Counts",
        "",
...
def write_figure_caption_pdf_offset_feasibility_reports(
    report: dict[str, Any], output_dir: str | Path
) -> dict[str, str]:
    root = Path(str(output_dir)).expanduser()
    root.mkdir(parents=True, exist_ok=True)
    report_path = root / "figure-caption-pdf-offset-feasibility.json"
    summary_path = root / "figure-caption-pdf-offset-feasibility-summary.json"
    markdown_path = root / "figure-caption-pdf-offset-feasibility.md"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    summary_path.write_text(json.dumps(_summary_payload(report), ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    markdown_path.write_text(render_figure_caption_pdf_offset_feasibility_markdown(report), encoding="utf-8")
    return {"report": str(report_path), "summary": str(summary_path), "markdown": str(markdown_path)}


def main(argv: list[str] | None = None) -> int:
    parser = ArgumentParser(description="Generate report-only FigureCaption original PDF offset feasibility.")
    parser.add_argument("--figure-caption-candidate-report", required=True)
    parser.add_argument("--pymupdf-parsed-root", required=True)
    parser.add_argument("--output-dir", default="", help="Directory for local JSON/Markdown reports.")
    parser.add_argument("--json", action="store_true", help="Print summary payload as JSON.")
    args = parser.parse_args(argv)

    report = build_figure_caption_pdf_offset_feasibility_report(
        figure_caption_candidate_report=args.figure_caption_candidate_report,
        pymupdf_parsed_root=args.pymupdf_parsed_root,
    )
    paths: dict[str, str] = {}
    if args.output_dir:
        paths = write_figure_caption_pdf_offset_feasibility_reports(report, args.output_dir)
...
__all__ = [
    ...
    "render_figure_caption_pdf_offset_feasibility_markdown",
