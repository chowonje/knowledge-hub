"""Apply-gated manifest writer for the StrictEvidence figure-caption pilot executor.

Consumes the figure-caption pilot executor dry-run report, validates figure-caption dry-run-ready
...
STRICT_EVIDENCE_FIGURE_CAPTION_PILOT_EXECUTOR_APPLY_SCHEMA_ID = (
    "knowledge-hub.paper.strict-evidence-figure-caption-pilot-executor-apply.v1"
)

APPLY_STATUS_READY = "apply_ready_figure_caption_pilot_validate_only"
APPLY_STATUS_APPLIED = "applied_figure_caption_pilot_manifest_only"
APPLY_STATUS_HELD_OUT = "held_out_section_tranche"
...
DEFAULT_EXECUTOR_DRY_RUN_REPORT_PATH = (
    ...
    / "strict-evidence-figure-caption-pilot-executor-dry-run.json"
)
...
DEFAULT_DRY_RUN_OUTPUT_DIR = (
    DEFAULT_REPORT_ROOT / "01-strict-evidence-figure-caption-pilot-executor-apply-dry-run"
)
DEFAULT_APPLY_OUTPUT_DIR = (
    DEFAULT_REPORT_ROOT / "02-strict-evidence-figure-caption-pilot-executor-apply"
)
...
def _classify_apply_row(
    ...
    *,
    ...
    if dry_run_status != DRY_RUN_STATUS_READY:
        ...
        if not _safe_bool(dry_row.get("dryRunReadyFigureCaptionPilotExecutorOnly")):
            blockers.append("dryRunReadyFigureCaptionPilotExecutorOnly_false")
        if not _safe_bool(dry_row.get("figureCaptionPilotCandidateOnly")):
            blockers.append("figureCaptionPilotCandidateOnly_false")
        if not _safe_bool(dry_row.get("figureCaptionTextOnly")):
...
def _apply_rows(
    ...
    *,
    ...
    for index, dry_row in enumerate(dry_rows):
        ...
        rows.append(
            {
                "apply_row_id": f"strict-evidence-figure-caption-pilot-executor-apply:{index:04d}",
                ...
                "figureCaptionPilotCandidateOnly": _safe_bool(source_row.get("figureCaptionPilotCandidateOnly")),
                ...
                "figureCaptionDryRunReady": ready or applied,
                ...
                "recommended_action": (
                    ...
                    if ready and not apply_mode
                        ...
                        if applied
                            ...
                            else "repair_dry_run_row_before_figure_caption_pilot_executor_apply"
                        )
                    )
                ),
            }
...
def _count_rows(
    *,
    rows: list[dict[str, Any]],
    input_schema_violations: list[str],
    figure_caption_dry_run_ready_rows: int,
    apply_mode: bool,
    manifest_write_rows: int,
) -> dict[str, Any]:
    section_ready = sum(1 for row in rows if row.get("figureCaptionDryRunReady"))
    return {
        "inputRows": len(rows),
        "figureCaptionDryRunReadyRows": section_ready,
        ...
        "figureCaptionPilotExecutorDryRunReadyRows": figure_caption_dry_run_ready_rows,
        "byPaperId": dict(
            Counter(
                _safe_text(row.get("paper_id"))
                for row in rows
                if row.get("figureCaptionDryRunReady")
            )
        ),
...
def build_strict_evidence_figure_caption_pilot_executor_apply(
    *,
    ...
    if dry_run_report:
        ...
        if not _safe_bool(gate.get("dryRunReadyForFigureCaptionPilotExecutor")):
    ...
    figure_caption_dry_run_ready_rows = int(
        (dry_run_report.get("counts") or {}).get("dryRunReadyFigureCaptionPilotExecutorOnlyRows")
        or 0
    ...
    run_id = _safe_text(run_id) or (
        f"strict-evidence-figure-caption-pilot-executor-apply-{_now_iso()}"
    )
    ...
    counts = _count_rows(
        rows=rows,
        input_schema_violations=input_schema_violations,
        figure_caption_dry_run_ready_rows=figure_caption_dry_run_ready_rows,
        apply_mode=bool(apply),
    ...
    figure_caption_ready_rows = int(counts.get("figureCaptionDryRunReadyRows") or 0)

    status = "ok"
    if (
        input_schema_violations
        or not rows
        or figure_caption_ready_rows != figure_caption_dry_run_ready_rows
        or figure_caption_ready_rows + held_out_rows != len(rows)
        or (bool(apply) and applied_manifest_rows != figure_caption_dry_run_ready_rows)
        or (not bool(apply) and planned_apply_rows != figure_caption_dry_run_ready_rows)
    ):
        status = "blocked"
    ...
    report = {
        "schema": STRICT_EVIDENCE_FIGURE_CAPTION_PILOT_EXECUTOR_APPLY_SCHEMA_ID,
        "status": status,
        ...
        "gate": {
            "readyForDryRunApplyPlanning": (
                status == "ok"
                and not apply
                and planned_apply_rows == figure_caption_dry_run_ready_rows
                and not input_schema_violations
            ),
            "readyForManifestOnlyApply": (
                status == "ok"
                and bool(figure_caption_dry_run_ready_rows)
                and not input_schema_violations
            ),
            ...
            "sectionRowsProcessedAsActiveApply": False,
            "schemaViolations": input_schema_violations,
            "decision": (
                "strict_evidence_figure_caption_pilot_executor_apply_ready"
                if status == "ok" and not apply
                else (
                    "strict_evidence_figure_caption_pilot_executor_applied_manifest_only"
                    if status == "ok" and apply
                    else "blocked"
                )
            ),
            "recommendedNextTranche": (
                "strict_evidence_pilot_tranche_manifest_readback_review"
                if status == "ok" and apply
                else "strict_evidence_figure_caption_pilot_executor_apply_review"
            ),
        },
        "policy": {
...
def render_strict_evidence_figure_caption_pilot_executor_apply_markdown(
    report: dict[str, Any],
) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- figure caption dry-run ready rows: {int(counts.get('figureCaptionDryRunReadyRows') or 0)}",
...
def write_strict_evidence_figure_caption_pilot_executor_apply_reports(
    report: dict[str, Any],
    output_dir: str | Path,
) -> dict[str, str]:
    root = Path(str(output_dir)).expanduser()
    root.mkdir(parents=True, exist_ok=True)
    report_path = root / "strict-evidence-figure-caption-pilot-executor-apply.json"
    summary_path = root / "strict-evidence-figure-caption-pilot-executor-apply-summary.json"
    markdown_path = root / "strict-evidence-figure-caption-pilot-executor-apply.md"
    ...
    markdown_path.write_text(
        render_strict_evidence_figure_caption_pilot_executor_apply_markdown(report),
        encoding="utf-8",
    )
...
def main(argv: list[str] | None = None) -> int:  # pragma: no cover
    parser = ArgumentParser(
        description=(
            "Dry-run or explicitly apply manifest-only figure-caption pilot executor outcomes "
    ...
    parser.add_argument(
        ...
        help="Path to figure-caption pilot executor dry-run JSON report.",
    )
    ...
    report = build_strict_evidence_figure_caption_pilot_executor_apply(
    ...
    paths = write_strict_evidence_figure_caption_pilot_executor_apply_reports(report, output_dir)
...
__all__ = [
    ...
    "APPLY_STATUS_READY",
    "STRICT_EVIDENCE_FIGURE_CAPTION_PILOT_EXECUTOR_APPLY_SCHEMA_ID",
    "build_strict_evidence_figure_caption_pilot_executor_apply",
    "default_output_dir",
    "render_strict_evidence_figure_caption_pilot_executor_apply_markdown",
    "resolve_output_dir",
    "write_strict_evidence_figure_caption_pilot_executor_apply_reports",
]
