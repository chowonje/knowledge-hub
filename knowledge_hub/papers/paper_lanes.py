...
_AI_LANE_FIELD_STRONG_PATTERNS = (
    re.compile(r"\bai\b"),
    re.compile(r"\bartificial intelligence\b"),
    re.compile(r"\bmachine learning\b"),
    re.compile(r"\bdeep learning\b"),
    re.compile(r"\bai systems?\b"),
    re.compile(r"\bcs\.(?:ai|cl|cv|ir|lg|ne|ro)\b"),
    re.compile(r"\bnlp\b"),
    re.compile(r"\bcomputer vision\b"),
    re.compile(r"\brobotics\b"),
)
_AI_LANE_FIELD_BROAD_PATTERNS = (
    re.compile(r"\bcomputer science\b"),
    re.compile(r"\bcs\.[a-z]+\b"),
)
_AI_LANE_TEXT_STRONG_PATTERNS = (
    re.compile(r"\bartificial intelligence\b"),
    re.compile(r"\bmachine learning\b"),
    re.compile(r"\bdeep learning\b"),
    re.compile(r"\bllms?\b"),
    re.compile(r"\blarge language models?\b"),
    re.compile(r"\blanguage models?\b"),
    re.compile(r"\bfoundation models?\b"),
    re.compile(r"\btransformers?\b"),
    re.compile(r"\bneural(?: networks?)?\b"),
    re.compile(r"\bdiffusion(?: models?)?\b"),
    re.compile(r"\breinforcement learning\b"),
    re.compile(r"\brlhf\b|\bgrpo\b|\bppo\b"),
    re.compile(r"\breasoning models?\b"),
    re.compile(r"\bchain[- ]of[- ]thought\b"),
    re.compile(r"\binstruction tuning\b|\bself[- ]?instruct\b"),
    re.compile(r"\bmultimodal\b|\bvision-language\b|\bvision language\b|\bvision-language-action\b"),
    re.compile(r"\bvla\b|\bvlm\b"),
    re.compile(r"\btext-to-(?:image|video|3d)\b"),
    re.compile(r"\bobject detection\b|\bimage segmentation\b|\bcaption(?:ing)?\b"),
    re.compile(r"\btool use\b|\bagentic\b|\bmulti-agent\b"),
    re.compile(r"\bagent benchmark(?:s)?\b|\bagent evaluation\b"),
    re.compile(r"\bembedding(?:s)?\b|\bretrieval-augmented\b|\brag\b"),
    re.compile(r"\bgpt[- ]?\d+\b|\bllama\b|\bqwen\b|\bdeepseek\b|\bphi[- ]?\d+\b|\bgemini\b|\bclaude\b|\bmamba\b"),
    re.compile(r"\bworld models?\b|\bmemory networks?\b"),
    re.compile(r"\bai\b"),
)
_AI_LANE_TEXT_SUPPORT_PATTERNS = (
    re.compile(r"\bmodels?\b"),
    re.compile(r"\blearning\b"),
    re.compile(r"\binference(?:-time)?\b"),
    re.compile(r"\btraining\b|\bpretrain(?:ing)?\b|\bfine[- ]?tuning\b"),
    re.compile(r"\breasoning\b"),
    re.compile(r"\brepresentation(?:s)?\b"),
    re.compile(r"\blatent\b"),
    re.compile(r"\breward\b|\bpolicy\b"),
    re.compile(r"\bself-supervised\b"),
    re.compile(r"\boptimization\b"),
)
...
def _combined_signal_text(paper: dict[str, Any], paper_memory: dict[str, Any] | None = None) -> str:
    ...
    if paper_memory:
        slot_bundle = shared_slot_payload(paper_memory)
        slots = dict(slot_bundle.get("slots") or {})
        parts.extend(
            [
                str(slots.get("overview") or paper_memory.get("paper_core") or paper_memory.get("paperCore") or ""),
                str(slots.get("method") or paper_memory.get("method_core") or paper_memory.get("methodCore") or ""),
                str(slots.get("evidence") or paper_memory.get("evidence_core") or paper_memory.get("evidenceCore") or ""),
                str(slots.get("limitations") or paper_memory.get("limitations") or ""),
                str(slot_bundle.get("search_text") or paper_memory.get("search_text") or paper_memory.get("searchText") or ""),
            ]
        )
...
def is_ai_lane_candidate(
    paper: dict[str, Any],
    paper_memory: dict[str, Any] | None = None,
    *,
    lane_scores: dict[str, int] | None = None,
) -> bool:
    field = str(paper.get("field") or "").lower()
    text = _combined_signal_text(paper, paper_memory)
    strong_field_hits = _pattern_hits(field, _AI_LANE_FIELD_STRONG_PATTERNS)
    broad_field_hits = _pattern_hits(field, _AI_LANE_FIELD_BROAD_PATTERNS)
    strong_text_hits = _pattern_hits(text, _AI_LANE_TEXT_STRONG_PATTERNS)
    support_text_hits = _pattern_hits(text, _AI_LANE_TEXT_SUPPORT_PATTERNS)
    scored_lane_hits = sum(1 for value in (lane_scores or {}).values() if int(value or 0) > 0)

    if strong_field_hits > 0 or strong_text_hits > 0:
        return True
    if broad_field_hits > 0 and support_text_hits >= 2:
        return True
    if scored_lane_hits >= 2 and support_text_hits > 0:
        return True
    if scored_lane_hits >= 1 and strong_field_hits > 0:
        return True
    return False
...
def seed_lane_metadata(
    ...
    if not is_ai_lane_candidate(paper, paper_memory, lane_scores=scores):
        return {
            "primary_lane": None,
            "secondary_tags": [],
