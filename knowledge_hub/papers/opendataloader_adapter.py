"""Optional OpenDataLoader PDF adapter for labs-only paper parsing."""
...
_FORMAT = "markdown,json"
ODL_READING_ORDER_CHOICES = ("auto", "xycut", "off")
ODL_TABLE_METHOD_CHOICES = ("auto", "default", "cluster")
...
def _document_meta_from_payload(payload: dict[str, Any]) -> dict[str, Any]:
    meta = {
        "file_name": _clean_text(payload.get("file name") or payload.get("file_name")),
        "page_count": _normalize_int(payload.get("number of pages") or payload.get("number_of_pages")),
        "title": _clean_text(payload.get("title")),
        "author": _clean_text(payload.get("author")),
        "creation_date": _clean_text(payload.get("creation date") or payload.get("creation_date")),
        "modification_date": _clean_text(payload.get("modification date") or payload.get("modification_date")),
    }
...
def normalize_opendataloader_convert_options(options: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    reading_order = _normalize_choice(
        payload.get("reading_order"),
        allowed=ODL_READING_ORDER_CHOICES,
        default="auto",
    )
    table_method = _normalize_choice(
        payload.get("table_method"),
        allowed=ODL_TABLE_METHOD_CHOICES,
        default="auto",
    )
    use_struct_tree = bool(payload.get("use_struct_tree"))
    normalized: dict[str, Any] = {}
    if reading_order != "auto":
        normalized["reading_order"] = reading_order
    if use_struct_tree:
        normalized["use_struct_tree"] = True
    if table_method != "auto":
        normalized["table_method"] = table_method
    return normalized


def resolve_opendataloader_convert_options(
    config: Any | None,
    *,
    ...
    merged = {
        "reading_order": _config_value("paper", "summary", "opendataloader", "reading_order", default="auto"),
        "use_struct_tree": _config_value("paper", "summary", "opendataloader", "use_struct_tree", default=False),
        "table_method": _config_value("paper", "summary", "opendataloader", "table_method", default="auto"),
    }
    for key, value in dict(overrides or {}).items():
        if key in merged and value is not None:
            merged[key] = value
    return normalize_opendataloader_convert_options(merged)
...
def _element_from_node(node: dict[str, Any]) -> dict[str, Any] | None:
    ...
    node_id = _normalize_int(node.get("id"))
    heading_level = _normalize_int(node.get("heading level") or node.get("heading_level"))
    linked_content_id = _normalize_int(node.get("linked content id") or node.get("linked_content_id"))
    row_number = _normalize_int(node.get("row number") or node.get("row_number"))
    column_number = _normalize_int(node.get("column number") or node.get("column_number"))
    row_span = _normalize_int(node.get("row span") or node.get("row_span"))
    column_span = _normalize_int(node.get("column span") or node.get("column_span"))
    hidden_text = _normalize_bool(node.get("hidden text") if "hidden text" in node else node.get("hidden_text"))
    child_count = len(node.get("kids")) if isinstance(node.get("kids"), list) else None
    source = _clean_text(node.get("source"))
    image_format = _clean_text(node.get("format") or node.get("image_format"))
...
def _dedupe_elements(elements: list[dict[str, Any]]) -> list[dict[str, Any]]:
    seen: set[tuple[int | None, str, str, int | None, str, int | None]] = set()
...
class OpenDataLoaderPDFAdapter:
    ...
    def _load_existing(
        ...
        *,
    ...
    ) -> OpenDataLoaderParseResult | None:
        ...
        stored_options = normalize_opendataloader_convert_options(
            parser_meta.get("convert_options") or payload.get("convert_options") or {}
        )
        if stored_options != normalize_opendataloader_convert_options(expected_options):
            return None
    ...
    def ensure_artifacts(
        ...
        *,
        ...
        effective_options = normalize_opendataloader_convert_options(parser_options)
        existing = None if refresh else self._load_existing(paper_id=token, expected_options=effective_options)
        ...
        with tempfile.TemporaryDirectory(prefix=f"khub-odl-{token}-") as tmp_dir:
            ...
            if generated_markdown is None or generated_json is None:
                raise RuntimeError("opendataloader conversion did not produce markdown/json outputs")
        ...
        document_meta = _document_meta_from_payload(raw_payload if isinstance(raw_payload, dict) else {})
        parser_meta = {
            "parser": "opendataloader",
            "mode": "local",
            "version": str(importlib.metadata.version("opendataloader-pdf")),
            "source_pdf": str(source_pdf),
            "convert_options": effective_options,
        }
        if document_meta:
            parser_meta["document_meta"] = document_meta
...
__all__ = [
    "ODL_READING_ORDER_CHOICES",
    "ODL_TABLE_METHOD_CHOICES",
    "OpenDataLoaderPDFAdapter",
    "OpenDataLoaderParseResult",
    "normalize_opendataloader_convert_options",
    "resolve_opendataloader_convert_options",
]
