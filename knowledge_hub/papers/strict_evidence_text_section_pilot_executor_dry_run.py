"""Dry-run planner for the StrictEvidence text-section pilot.

Consumes the text-section pilot report, validates section candidate rows, and emits
deterministic validate-only executor plans without mutating records or integration
surfaces.
"""
...
from knowledge_hub.papers.strict_evidence_text_section_pilot import (
    PILOT_STATUS_CANDIDATE_ONLY,
    PILOT_STATUS_HELD_OUT_NON_SECTION,
    STRICT_EVIDENCE_TEXT_SECTION_PILOT_SCHEMA_ID,
)


STRICT_EVIDENCE_TEXT_SECTION_PILOT_EXECUTOR_DRY_RUN_SCHEMA_ID = (
    "knowledge-hub.paper.strict-evidence-text-section-pilot-executor-dry-run.v1"
)

DRY_RUN_STATUS_READY = "dry_run_ready_text_section_pilot_executor_only"
DRY_RUN_STATUS_HELD_OUT = "held_out_non_section_tranche"
DRY_RUN_STATUS_BLOCKED_INPUT_SCHEMA = "blocked_input_schema_violation"
DRY_RUN_STATUS_BLOCKED_PILOT_NOT_CANDIDATE = "blocked_section_pilot_not_candidate"
...
PLANNED_ACTION_VALIDATE_ONLY = "section_strict_evidence_pilot_validate_only"
PLANNED_WRITE_TARGET_NONE = "none"
PLANNED_RUNTIME_EFFECT_NONE = "none"
PLANNED_ANSWER_EFFECT_NONE = "none"

DEFAULT_SECTION_PILOT_REPORT_PATH = (
    Path.home()
    / ".khub"
    / "reports"
    / "layout-parser-pilot"
    / "2026-05-19"
    / "strict-evidence-text-section-pilot"
    / "01-strict-evidence-text-section-pilot"
    / "strict-evidence-text-section-pilot.json"
)
...
def _classify_dry_run_row(
    ...
    *,
    ...
    if pilot_status == PILOT_STATUS_HELD_OUT_NON_SECTION:
    ...
    if pilot_status != PILOT_STATUS_CANDIDATE_ONLY:
        ...
        if not _safe_bool(pilot_row.get("textSectionPilotCandidateOnly")):
            blockers.append("textSectionPilotCandidateOnly_false")
...
def _planned_executor_key(pilot_row: dict[str, Any]) -> str:
    ...
    if strict_evidence_id:
        return f"section-pilot-executor:{strict_evidence_id}"
    return f"section-pilot-executor:{idempotency_key or 'unknown'}"
        ...
        rows.append(
            {
                ...
                "textSectionPilotCandidateOnly": _safe_bool(
                    source_row.get("textSectionPilotCandidateOnly")
                ),
                ...
                "recommended_action": (
                    "queue_for_strict_evidence_text_section_pilot_executor_apply"
                    if ready
                    else (
                        "held_for_figure_caption_pilot_tranche"
                        if dry_run_status == DRY_RUN_STATUS_HELD_OUT
                        else "repair_section_pilot_row_before_executor_dry_run"
                    )
                ),
            }
        )
...
def _count_rows(
    *,
    ...
    section_pilot_candidate_rows: int,
) -> dict[str, Any]:
    return {
        "inputRows": len(rows),
        "sectionPilotCandidateRows": section_pilot_candidate_rows,
        "dryRunReadyTextSectionPilotExecutorOnlyRows": sum(
        ...
        "blockedSectionPilotNotCandidateRows": sum(
            1 for row in rows if row.get("dry_run_status") == DRY_RUN_STATUS_BLOCKED_PILOT_NOT_CANDIDATE
        ),
...
def build_strict_evidence_text_section_pilot_executor_dry_run(
    *,
    section_pilot_report_path: str | Path = DEFAULT_SECTION_PILOT_REPORT_PATH,
    paper_ids: list[str] | None = None,
) -> dict[str, Any]:
    report_path = Path(str(section_pilot_report_path)).expanduser()
    warnings: list[str] = []
    input_schema_violations: list[str] = []
    requested_papers = {str(item).strip() for item in (paper_ids or []) if str(item).strip()}

    section_pilot_report = _read_json(report_path)
    if not section_pilot_report:
        warnings.append("section_pilot_report_missing_or_unreadable")

    validation = validate_payload(
        section_pilot_report,
        STRICT_EVIDENCE_TEXT_SECTION_PILOT_SCHEMA_ID,
        strict=True,
    )
    if not validation.ok:
        ...
        if not section_pilot_report:
            input_schema_violations.append("section_pilot_report_missing_or_unreadable")

    if section_pilot_report and _safe_text(section_pilot_report.get("status")) != "ok":
        input_schema_violations.append(
            f"section_pilot_report_status={_safe_text(section_pilot_report.get('status')) or 'unknown'}"
        )

    all_pilot_rows = [
        row for row in section_pilot_report.get("rows", []) if isinstance(row, dict)
    ] if isinstance(section_pilot_report, dict) else []

    section_pilot_candidate_rows = int(
        (section_pilot_report.get("counts") or {}).get("textSectionPilotCandidateOnlyRows")
        or sum(
            1
            for row in all_pilot_rows
            if _safe_bool(row.get("textSectionPilotCandidateOnly"))
            and _safe_text(row.get("pilot_status")) == PILOT_STATUS_CANDIDATE_ONLY
        )
    ) if section_pilot_report else 0
    ...
    if not all_pilot_rows and not input_schema_violations:
        warnings.append("section_pilot_rows_missing")
    ...
    if input_schema_violations:
        for row in rows:
            ...
            row["dryRunReadyTextSectionPilotExecutorOnly"] = False
            row["plannedAction"] = ""
            row["recommended_action"] = "repair_section_pilot_report_schema_before_executor_dry_run"

    counts = _count_rows(
        rows=rows,
        input_schema_violations=_dedupe(input_schema_violations),
        section_pilot_candidate_rows=section_pilot_candidate_rows,
    )
    ready_rows = int(counts.get("dryRunReadyTextSectionPilotExecutorOnlyRows") or 0)
    held_out_rows = int(counts.get("heldOutNonSectionRows") or 0)
    status = "ok"
    if (
        input_schema_violations
        or not rows
        or ready_rows != section_pilot_candidate_rows
    ...
    return {
        ...
        "input": {
            "sectionPilotReportPath": str(report_path),
            "sectionPilotSchema": _safe_text(section_pilot_report.get("schema")) if section_pilot_report else "",
            "sectionPilotReportStatus": _safe_text(section_pilot_report.get("status")) if section_pilot_report else "",
        ...
        "gate": {
            "dryRunReadyForTextSectionPilotExecutor": (
                bool(ready_rows)
                and ready_rows == section_pilot_candidate_rows
            ...
            "decision": (
                "strict_evidence_text_section_pilot_executor_dry_run_ready"
                if status == "ok"
                else "blocked"
            ),
            "recommendedNextTranche": (
                ...
                if status == "ok"
                else "strict_evidence_text_section_pilot_repair"
            ),
        },
        "policy": {
            "reportOnly": True,
...
def render_strict_evidence_text_section_pilot_executor_dry_run_markdown(
    report: dict[str, Any],
) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- dry-run ready section rows: {int(counts.get('dryRunReadyTextSectionPilotExecutorOnlyRows') or 0)}",
...
def write_strict_evidence_text_section_pilot_executor_dry_run_reports(
    report: dict[str, Any],
    output_dir: str | Path,
) -> dict[str, str]:
    root = Path(str(output_dir)).expanduser()
    root.mkdir(parents=True, exist_ok=True)
    report_path = root / "strict-evidence-text-section-pilot-executor-dry-run.json"
    summary_path = root / "strict-evidence-text-section-pilot-executor-dry-run-summary.json"
    markdown_path = root / "strict-evidence-text-section-pilot-executor-dry-run.md"
    ...
    markdown_path.write_text(
        render_strict_evidence_text_section_pilot_executor_dry_run_markdown(report),
        encoding="utf-8",
    )
...
def main(argv: list[str] | None = None) -> int:  # pragma: no cover
    parser = ArgumentParser(
        description=(
            "Plan validate-only text-section pilot executor actions from section-pilot "
            "rows without mutating records or integration surfaces."
        )
    )
    parser.add_argument(
        "--section-pilot-report",
        default=str(DEFAULT_SECTION_PILOT_REPORT_PATH),
        help="Text-section pilot JSON report.",
    )
    ...
    report = build_strict_evidence_text_section_pilot_executor_dry_run(
        section_pilot_report_path=args.section_pilot_report,
        paper_ids=args.paper_id or None,
    )

    if args.output_dir:
        paths = write_strict_evidence_text_section_pilot_executor_dry_run_reports(
            report,
            args.output_dir,
        )
...
__all__ = [
    ...
    "render_strict_evidence_text_section_pilot_executor_dry_run_markdown",
    "write_strict_evidence_text_section_pilot_executor_dry_run_reports",
]
