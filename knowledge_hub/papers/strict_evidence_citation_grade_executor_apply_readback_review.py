"""Readback review for applied StrictEvidence citation-grade store records.

Reconciles citation-grade JSONL rows written by the citation-grade executor apply
tranche against StrictEvidence, eligibility, and SourceSpan stores. Read-only:
does not write citation-grade, eligibility, StrictEvidence, or SourceSpan JSONL or
...
STRICT_EVIDENCE_CITATION_GRADE_EXECUTOR_APPLY_READBACK_REVIEW_SCHEMA_ID = (
    "knowledge-hub.paper.strict-evidence-citation-grade-executor-apply-readback-review.v1"
)

READBACK_STATUS_VALIDATED = "citation_grade_readback_validated"
READBACK_STATUS_BLOCKED_MISSING_STORE = "blocked_missing_citation_grade_store"
READBACK_STATUS_BLOCKED_SCHEMA = "blocked_citation_grade_record_schema_violation"
READBACK_STATUS_BLOCKED_SEMANTIC = "blocked_citation_grade_record_semantic_violation"
...
READBACK_STATUS_BLOCKED_DUPLICATE_RECORD_ID = "blocked_duplicate_citation_grade_record_id"
...
DEFAULT_APPLY_REPORT_PATH = (
    ...
    / "02-strict-evidence-citation-grade-executor-apply"
    / "strict-evidence-citation-grade-executor-apply.json"
)
...
DEFAULT_DRY_RUN_REPORT_PATH = (
    ...
    / "2026-05-20"
    / "strict-evidence-citation-grade-executor-apply"
    / "01-strict-evidence-citation-grade-executor-apply-dry-run"
    / "strict-evidence-citation-grade-executor-apply.json"
)
...
DEFAULT_OUTPUT_DIR = (
    ...
    / "layout-parser-pilot"
    / "2026-05-20"
    / "strict-evidence-citation-grade-executor-apply-readback-review"
    / "01-strict-evidence-citation-grade-executor-apply-readback-review"
)
...
def _extended_semantic_violations(record: dict[str, Any]) -> list[str]:
    violations = list(validate_citation_grade_record_semantics(record))
    if _safe_text(record.get("citationGradeState")) != CITATION_GRADE_STATE_CANDIDATE_ONLY:
        violations.append(
            f"citationGradeState_mismatch:{_safe_text(record.get('citationGradeState')) or 'missing'}"
        )
    if _safe_text(record.get("citationGradePolicyVersion")) != CITATION_GRADE_POLICY_VERSION:
        violations.append("citationGradePolicyVersion_mismatch")
    if _safe_text(record.get("citationGradeDecision")) != CITATION_GRADE_DECISION:
        violations.append("citationGradeDecision_mismatch")
    return _dedupe(violations)
...
def _candidate_identity_violations(
    ...
    *,
    ...
    for field_name in ("sourceContent", "sourceContentHash"):
        ...
        for label, parent in (
            ...
            parent_value = _safe_text(parent.get(field_name))
            if parent_value and parent_value != record_value:
                violations.append(f"{label}_{field_name}_mismatch")
...
def _classify_citation_grade_record(
    record: dict[str, Any],
    *,
    ...
    if not store_exists:
        return READBACK_STATUS_BLOCKED_MISSING_STORE, ["citation_grade_store_missing"]

    flag_violations = _runtime_or_citation_violations(record)
    if flag_violations:
        return READBACK_STATUS_BLOCKED_RUNTIME_OR_CITATION, flag_violations

    validation = validate_payload(record, STRICT_EVIDENCE_CITATION_GRADE_RECORD_SCHEMA_ID, strict=True)
    ...
    if duplicate_record_id:
        return READBACK_STATUS_BLOCKED_DUPLICATE_RECORD_ID, [
            f"duplicate_citationGradeRecordId={_safe_text(record.get('citationGradeRecordId'))}"
        ]
    ...
    if strict_parent_violations:
        return READBACK_STATUS_BLOCKED_RUNTIME_OR_CITATION, strict_parent_violations
    ...
    if eligibility_parent_violations:
        return READBACK_STATUS_BLOCKED_RUNTIME_OR_CITATION, eligibility_parent_violations
    ...
    if source_span_parent_violations:
        return READBACK_STATUS_BLOCKED_RUNTIME_OR_CITATION, source_span_parent_violations
...
def build_strict_evidence_citation_grade_executor_apply_readback_review(
    *,
    ...
    store_exists = citation_grade_root.is_dir()

    citation_grade_raw = _iter_jsonl_records(citation_grade_root)
    if requested_papers:
        citation_grade_raw = [
            item
            for item in citation_grade_raw
    ...
    for item in citation_grade_raw:
        ...
        record_id = _safe_text(record.get("citationGradeRecordId"))
        if idempotency_key:
    ...
    apply_by_strict_evidence_id = {
        _safe_text(row.get("strictEvidenceId")): row
        for row in apply_rows
        if _safe_text(row.get("strictEvidenceId"))
    }

    rows: list[dict[str, Any]] = []
    for index, item in enumerate(citation_grade_raw):
        ...
        readback_status, blockers = _classify_citation_grade_record(
            record,
        ...
        rows.append(
            {
                "readback_row_id": (
                    f"strict-evidence-citation-grade-executor-apply-readback-review:{index:04d}"
                ),
                ...
                "citation_grade_store_path": _safe_text(item.get("record_path")),
                "citation_grade_store_line": int(item.get("record_line") or 0),
                "plannedWriteTarget": _safe_text(record.get("plannedWriteTarget")),
                "citationGradePolicyVersion": _safe_text(record.get("citationGradePolicyVersion")),
                "citationGradeDecision": _safe_text(record.get("citationGradeDecision")),
                "citationGradeState": _safe_text(record.get("citationGradeState")),
                "readback_status": readback_status,
                "readback_blockers": _dedupe(blockers),
                "citationGradeReadbackValidated": readback_status == READBACK_STATUS_VALIDATED,
                ...
                "recommended_action": (
                    "queue_for_strict_evidence_citation_grade_post_apply_promotion_hold_review"
                    if readback_status == READBACK_STATUS_VALIDATED
                    else "repair_citation_grade_record_before_post_apply_promotion"
                ),
            }
        )

    citation_grade_record_rows = len(citation_grade_raw)
    ...
    if citation_grade_record_rows != expected_citation_grade_record_rows:
        schema_violations.append(
            "citation_grade_record_rows="
            f"{citation_grade_record_rows}:expected={expected_citation_grade_record_rows}"
        )
    ...
    counts["citationGradeRecordRows"] = citation_grade_record_rows
    ...
    if (
        ...
        or citation_grade_record_rows != expected_citation_grade_record_rows
        or readback_validated_rows != expected_citation_grade_record_rows
        or readback_validated_rows != len(rows)
    ):
        status = "blocked"

    return {
        "schema": STRICT_EVIDENCE_CITATION_GRADE_EXECUTOR_APPLY_READBACK_REVIEW_SCHEMA_ID,
        "status": status,
        ...
        "gate": {
            ...
            "decision": (
                "strict_evidence_citation_grade_executor_apply_readback_review_ready"
                if status == "ok"
                else "strict_evidence_citation_grade_executor_apply_readback_review_blocked"
            ),
            "recommendedNextTranche": (
                "strict_evidence_citation_grade_post_apply_promotion_hold_review"
                if status == "ok"
                else "strict_evidence_citation_grade_executor_apply_repair"
            ),
        },
        "policy": {
            "readbackOnly": True,
...
def _count_rows(
    *,
    ...
    return {
        ...
        "citationGradeRecordRows": len(rows),
        "readbackValidatedRows": int(by_status.get(READBACK_STATUS_VALIDATED, 0)),
        "blockedMissingCitationGradeStoreRows": int(by_status.get(READBACK_STATUS_BLOCKED_MISSING_STORE, 0)),
        "blockedCitationGradeRecordSchemaViolationRows": int(by_status.get(READBACK_STATUS_BLOCKED_SCHEMA, 0)),
        "blockedCitationGradeRecordSemanticViolationRows": int(
        ...
        "blockedDuplicateCitationGradeRecordIdRows": int(
        ...
        "byPaperId": dict(
            Counter(_safe_text(row.get("paper_id")) for row in rows if row.get("citationGradeReadbackValidated"))
        ),
        "byArtifactType": dict(
            Counter(
                _safe_text(row.get("artifact_type"))
                for row in rows
                if row.get("citationGradeReadbackValidated")
            )
        ),
...
def render_strict_evidence_citation_grade_executor_apply_readback_review_markdown(
    report: dict[str, Any],
) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- citation-grade boolean mutations: {int(counts.get('citationGradeBooleanMutationRows') or 0)}",
            "",
...
def write_strict_evidence_citation_grade_executor_apply_readback_review_reports(
    report: dict[str, Any],
    output_dir: str | Path,
) -> dict[str, str]:
    root = Path(str(output_dir)).expanduser()
    root.mkdir(parents=True, exist_ok=True)
    report_path = root / "strict-evidence-citation-grade-executor-apply-readback-review.json"
    summary_path = root / "strict-evidence-citation-grade-executor-apply-readback-review-summary.json"
    markdown_path = root / "strict-evidence-citation-grade-executor-apply-readback-review.md"
    ...
    markdown_path.write_text(
        render_strict_evidence_citation_grade_executor_apply_readback_review_markdown(report),
        encoding="utf-8",
    )
...
def main(argv: list[str] | None = None) -> int:  # pragma: no cover
    parser = ArgumentParser(
        description=(
            "Read-only readback review for applied StrictEvidence citation-grade JSONL records."
        )
    )
    parser.add_argument(
        "--apply-report",
        default=str(DEFAULT_APPLY_REPORT_PATH),
        help="Path to citation-grade executor apply JSON report.",
    )
    parser.add_argument(
        "--dry-run-report",
        default=str(DEFAULT_DRY_RUN_REPORT_PATH),
        help="Path to citation-grade executor apply dry-run plan JSON report.",
    )
    parser.add_argument(
        "--contract-report",
    ...
    parser.add_argument(
        ...
        help=(
            "Local papers_dir root for citation-grade/strict_evidence/"
    ...
    report = build_strict_evidence_citation_grade_executor_apply_readback_review(
        ...
        citation_grade_record_contract_report_path=args.contract_report,
...
__all__ = [
    ...
    "DEFAULT_OUTPUT_DIR",
    "DEFAULT_PAPERS_DIR",
    "EXPECTED_CITATION_GRADE_RECORD_ROWS",
    ...
    "render_strict_evidence_citation_grade_executor_apply_readback_review_markdown",
