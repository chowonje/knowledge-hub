"""Post-merge convergence for the KnowledgeOS v0.1 RC controlled execution PR."""
...
from knowledge_hub.papers.knowledgeos_v01_rc_corpus_scale_answer_quality_controlled_execution import (
    BLOCKED_DECISION as CONTROLLED_EXECUTION_BLOCKED_DECISION,
...
KNOWLEDGEOS_V01_RC_CORPUS_SCALE_CONTROLLED_EXECUTION_POST_MERGE_CONVERGENCE_SCHEMA_ID = (
    "knowledge-hub.product.knowledgeos-v01-rc-corpus-scale-controlled-execution-post-merge-convergence.v1"
)

READY_DECISION = "knowledgeos_v01_rc_corpus_scale_controlled_execution_post_merge_convergence_ready"
BLOCKED_DECISION = "knowledgeos_v01_rc_corpus_scale_controlled_execution_post_merge_convergence_blocked"
NEXT_TRANCHE_READY = "corpus_scale_answer_quality_answerability_gate_repair"
NEXT_TRANCHE_BLOCKED = "corpus_scale_controlled_execution_post_merge_convergence_repair"
...
DEFAULT_PR_NUMBER = 176
DEFAULT_HEAD_BRANCH = "codex/corpus-scale-answer-quality-controlled-execution-20260529"
...
SELF_REVIEW_ALLOWED_DIRTY_PATHS = {
    ...
    "docs/schemas/knowledgeos-v01-rc-corpus-scale-controlled-execution-post-merge-convergence.v1.json",
    "eval/knowledgeos/reports/knowledgeos_v01_rc_corpus_scale_controlled_execution_post_merge_convergence.v1.json",
    "eval/knowledgeos/reports/knowledgeos_v01_rc_corpus_scale_controlled_execution_post_merge_convergence.v1.md",
    "eval/knowledgeos/scripts/build_knowledgeos_v01_rc_corpus_scale_controlled_execution_post_merge_convergence.py",
    "knowledge_hub/core/schema_validator.py",
    "knowledge_hub/papers/knowledgeos_v01_rc_corpus_scale_controlled_execution_post_merge_convergence.py",
    "tests/test_knowledgeos_v01_rc_corpus_scale_controlled_execution_post_merge_convergence.py",
}
...
def _controlled_execution_blockers(report: dict[str, Any]) -> list[str]:
    ...
    if report.get("status") != "blocked":
        blockers.append("controlled_execution_not_blocked_as_expected")
    if report.get("decision") != CONTROLLED_EXECUTION_BLOCKED_DECISION:
        blockers.append("controlled_execution_decision_not_blocked")
    if report.get("nextRecommendedTranche") != NEXT_TRANCHE_READY:
        blockers.append("controlled_execution_next_tranche_not_answerability_repair")
    ...
    if _int(counts.get("executionFailRows")) <= 0:
        blockers.append("controlled_execution_failures_missing")
    if _int(counts.get("unexpectedAnswerableRows")) <= 0:
        blockers.append("controlled_execution_unexpected_answerable_rows_missing")
    if _int(counts.get("noAnswerSafetyFailRows")) <= 0:
        blockers.append("controlled_execution_no_answer_safety_fail_rows_missing")
...
def build_knowledgeos_v01_rc_corpus_scale_controlled_execution_post_merge_convergence(
    *,
    ...
    if private_path_leak_rows:
        semantic_violations.append("controlled_execution_post_merge_private_path_marker")
    ...
    counts = {
        ...
        "controlledExecutionBlockedRows": 1 if controlled_report.get("status") == "blocked" else 0,
        ...
        "answerabilityGateRepairRequiredRows": 1 if status == "ready" else 0,
    ...
    return {
        "schema": KNOWLEDGEOS_V01_RC_CORPUS_SCALE_CONTROLLED_EXECUTION_POST_MERGE_CONVERGENCE_SCHEMA_ID,
        "status": status,
        ...
        "postMergeDecision": {
            "postMergeConvergence": "controlled_execution_merged" if status == "ready" else "blocked",
            ...
            "generalRcDecision": "blocked_pending_answerability_gate_repair",
            "nextGate": NEXT_TRANCHE_READY if status == "ready" else NEXT_TRANCHE_BLOCKED,
        },
        "counts": counts,
        "gate": {
            "postMergeConvergenceReady": status == "ready",
            "controlledExecutionBlockedAsExpected": not controlled_blockers,
            ...
            "answerabilityGateRepairRequired": counts["answerabilityGateRepairRequiredRows"] == 1,
        ...
        "checkRows": [
            ...
            {"checkId": "github_pr_176", "status": "pass" if not pr_blockers else "fail", "blockers": pr_blockers},
        ...
        "warnings": [
            "controlled_execution_blocked_is_expected_and_drives_answerability_gate_repair",
...
def render_knowledgeos_v01_rc_corpus_scale_controlled_execution_post_merge_convergence_markdown(
    report: dict[str, Any],
) -> str:
    counts = dict(report.get("counts") or {})
    ...
    lines = [
        ...
        f"- answerabilityGateRepairRequiredRows: `{counts.get('answerabilityGateRepairRequiredRows')}`",
...
def write_knowledgeos_v01_rc_corpus_scale_controlled_execution_post_merge_convergence(
    report: dict[str, Any],
    *,
    report_json: Path,
    report_md: Path,
    ...
    report_md.write_text(
        render_knowledgeos_v01_rc_corpus_scale_controlled_execution_post_merge_convergence_markdown(report),
        encoding="utf-8",
    )
