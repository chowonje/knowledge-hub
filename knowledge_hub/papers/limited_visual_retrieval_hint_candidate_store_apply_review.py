"""Report-only review gate for limited visual retrieval-hint apply design.

This helper consumes the limited apply-design report and checks that every
planned row is still safe to send to a future apply-executor dry-run. It does
not write candidate-store JSONL, index vectors, promote evidence, or expose
visual derived text at answer runtime.
"""
...
LIMITED_VISUAL_RETRIEVAL_HINT_CANDIDATE_STORE_APPLY_REVIEW_SCHEMA_ID = (
    "knowledge-hub.paper.limited-visual-retrieval-hint-candidate-store-apply-review.v1"
)
LIMITED_VISUAL_RETRIEVAL_HINT_CANDIDATE_STORE_APPLY_REVIEW_ROW_SCHEMA_ID = (
    "knowledge-hub.paper.limited-visual-retrieval-hint-candidate-store-apply-review-row.v1"
)

READY_DECISION = "ready_for_limited_visual_retrieval_hint_candidate_store_apply_executor_dry_run"
BLOCKED_DECISION = "blocked"
NEXT_RECOMMENDED_TRANCHE = "limited_visual_retrieval_hint_candidate_store_apply_executor_dry_run"
...
def _review_row(row: dict[str, Any]) -> dict[str, Any]:
    checks = _checks(row)
    blocker_reasons = [name for name, passed in checks.items() if not passed]
    review_ready = not blocker_reasons
    basis = "|".join(
        [
    ...
    return {
        "schema": LIMITED_VISUAL_RETRIEVAL_HINT_CANDIDATE_STORE_APPLY_REVIEW_ROW_SCHEMA_ID,
        "reviewRowId": "limited-visual-retrieval-hint-apply-review:" + _short_hash(basis),
        ...
        "reviewStatus": "ready_for_apply_executor_dry_run" if review_ready else "blocked",
        "checks": checks,
        "searchEvalSummary": dict(row.get("searchEvalSummary") or {}),
        "reviewPlan": {
            "applyExecutorDryRunCandidate": review_ready,
        ...
        output.append(
            {
                ...
                "applyExecutorDryRunCandidateRows": sum(
                    1 for row in items if row.get("reviewPlan", {}).get("applyExecutorDryRunCandidate")
                ),
...
def _counts(
    ...
    *,
    ...
    return {
        ...
        "reviewRows": len(rows),
        "reviewReadyRows": sum(1 for row in rows if row.get("reviewStatus") == "ready_for_apply_executor_dry_run"),
        "applyExecutorDryRunCandidateRows": sum(
            1 for row in rows if row.get("reviewPlan", {}).get("applyExecutorDryRunCandidate")
        ),
...
def build_limited_visual_retrieval_hint_candidate_store_apply_review(
    ...
    *,
    ...
    if apply_design_report.get("status") != "ready":
        source_blockers.append("apply_design_not_ready")
    if apply_design_report.get("decision") != "ready_for_limited_visual_retrieval_hint_candidate_store_apply_review":
        source_blockers.append("apply_design_not_ready_for_review")
    if int(counts.get("blockedRows") or 0) != 0:
        source_blockers.append("apply_design_has_blocked_rows")
    if int(counts.get("candidateStoreWriteRows") or 0) != 0:
        source_blockers.append("apply_design_has_store_writes")
    if int(counts.get("limitedApplyDesignCandidateRows") or 0) != int(counts.get("applyDesignRows") or 0):
        source_blockers.append("apply_design_candidate_count_mismatch")
    ...
    report: dict[str, Any] = {
        "schema": LIMITED_VISUAL_RETRIEVAL_HINT_CANDIDATE_STORE_APPLY_REVIEW_SCHEMA_ID,
        "status": "ready",
    ...
    if (
        ...
        or report_counts["reviewReadyRows"] != report_counts["reviewRows"]
        or not rows
    ):
...
def render_markdown_report(report: dict[str, Any]) -> str:
    ...
    for row in report.get("typeSummary", []):
        lines.append(
            "| {candidateType} | {rows} | {candidates} | {improved} | {strong} | {blocked} |".format(
                ...
                candidates=row.get("applyExecutorDryRunCandidateRows"),
    ...
    for index, row in enumerate(list(report.get("reviewRowsDetail") or [])[:24], start=1):
        ...
        lines.append(
            "| {index} | {paperId} | {candidateType} | {ready} | {improved} | {strong} | `{sourceId}` |".format(
                index=index,
                paperId=row.get("paperId"),
                candidateType=row.get("candidateType"),
                ready=row.get("reviewPlan", {}).get("applyExecutorDryRunCandidate"),
...
def write_limited_visual_retrieval_hint_candidate_store_apply_review(
    report: dict[str, Any],
    *,
    report_json: Path,
    report_md: Path,
