"""Readback review for applied parsed-artifact SourceSpan store records.

This helper reads local SourceSpan JSONL files written by the promotion
executor apply tranche, validates records against the SourceSpan record schema,
and confirms they remain non-strict and non-runtime before any later
strict-evidence gate. It does not write SourceSpan records or mutate candidate
stores, databases, indexes, vault content, or runtime evidence surfaces.
"""
...
PARSED_ARTIFACT_SOURCE_SPAN_PROMOTION_READBACK_REVIEW_SCHEMA_ID = (
    "knowledge-hub.paper.parsed-artifact-source-span-promotion-readback-review.v1"
)

READBACK_STATUS_VALIDATED = "readback_validated_source_span"
...
READBACK_STATUS_BLOCKED_UNSUPPORTED_ARTIFACT_TYPE = "blocked_unsupported_artifact_type"
READBACK_STATUS_BLOCKED_UNSUPPORTED_TARGET = "blocked_unsupported_source_span_target"

ALLOWED_ARTIFACT_TYPES = set(SOURCE_SPAN_STORE_CONTRACT.get("allowedArtifactTypes") or [])
...
def _classify_record(
    ...
    *,
    ...
    if _safe_text(record.get("plannedWriteTarget")) != PARSED_ARTIFACT_SOURCE_SPAN_STORE:
    ...
    if not _safe_text(record.get("sourceSpanId")):
        blockers.append("sourceSpanId_missing")
        return READBACK_STATUS_BLOCKED_SCHEMA_VIOLATION, blockers

    if not _safe_text(record.get("candidateRecordId")):
        blockers.append("candidateRecordId_missing")
        return READBACK_STATUS_BLOCKED_SCHEMA_VIOLATION, blockers

    if not _safe_text(record.get("paperId")):
        blockers.append("paperId_missing")
        return READBACK_STATUS_BLOCKED_SCHEMA_VIOLATION, blockers

    artifact_type = _safe_text(record.get("artifactType"))
    if artifact_type not in ALLOWED_ARTIFACT_TYPES:
        return READBACK_STATUS_BLOCKED_UNSUPPORTED_ARTIFACT_TYPE, [
            f"artifactType={artifact_type or 'unknown'}"
        ]

    record_run_id = _safe_text(record.get("runId"))
    if requested_run_id and record_run_id != requested_run_id:
        return READBACK_STATUS_BLOCKED_SCHEMA_VIOLATION, [
    ...
    for index, raw_row in enumerate(raw_rows):
        ...
        if status == READBACK_STATUS_BLOCKED_SCHEMA_VIOLATION:
            schema_violations.extend(
                f"source_span_record_schema_violation:{record.get('sourceSpanId') or index}:{error}"
        ...
        rows.append(
            {
                "review_row_id": (
                    f"parsed-artifact-source-span-promotion-readback-review:{index:04d}"
                ),
                ...
                "idempotencyKey": key,
                "source_span_store_path": _safe_text(raw_row.get("record_path")),
                "source_span_store_line": _safe_int(raw_row.get("record_line")) or 0,
                ...
                "recommended_action": (
                    ...
                    else "repair_source_span_store_record_before_strict_evidence_gate"
                ),
            }
        )
...
def build_parsed_artifact_source_span_promotion_readback_review(
    *,
    ...
    status = "ok"
    validated_rows = int(counts.get("readbackValidatedRows") or 0)
    if schema_violations or not rows or validated_rows != len(rows):
        status = "blocked"
    ...
    return {
        ...
        "counts": counts,
        "gate": {
            "readyForStrictEvidenceGate": bool(validated_rows)
            and validated_rows == len(rows)
            ...
            "decision": (
                "parsed_artifact_source_span_promotion_readback_review_ready"
                if status == "ok"
                else "blocked"
            ),
...
def render_parsed_artifact_source_span_promotion_readback_review_markdown(
    report: dict[str, Any],
) -> str:
    ...
    by_paper = [
        f"{paper_id}: {count}"
        for paper_id, count in sorted((dict(counts.get("byPaperId") or {})).items())
    ]
    return "\n".join(
        [
            "# Parsed Artifact SourceSpan Promotion Readback Review",
            "",
...
def write_parsed_artifact_source_span_promotion_readback_review_reports(
    report: dict[str, Any],
    output_dir: str | Path,
) -> dict[str, str]:
    root = Path(str(output_dir)).expanduser()
    root.mkdir(parents=True, exist_ok=True)
    report_path = root / "parsed-artifact-source-span-promotion-readback-review.json"
    summary_path = root / "parsed-artifact-source-span-promotion-readback-review-summary.json"
    markdown_path = root / "parsed-artifact-source-span-promotion-readback-review.md"
    ...
    markdown_path.write_text(
        render_parsed_artifact_source_span_promotion_readback_review_markdown(report),
        encoding="utf-8",
    )
...
def main(argv: list[str] | None = None) -> int:  # pragma: no cover
    parser = ArgumentParser(
        description=(
            "Review applied parsed-artifact SourceSpan JSONL records before any "
            "strict-evidence promotion gate."
        )
    )
